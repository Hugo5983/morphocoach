...
export function statusBadge(pct) {
  if (pct >= 95) return { label:"✅ Excellent",       color:"#34D399", bg:"rgba(52,211,153,0.12)",  bd:"rgba(52,211,153,0.25)" };
  if (pct >= 80) return { label:"🔶 Peut mieux faire", color:"#FBBF24", bg:"rgba(251,191,36,0.12)",  bd:"rgba(251,191,36,0.25)" };
  return              { label:"⚠️ À améliorer",       color:"#F87171", bg:"rgba(248,113,113,0.12)", bd:"rgba(248,113,113,0.25)" };
}
...
export function MacroRow({ label, color, value, goal, pct, partial }) {
  ...
  const COLORS = {
    Protéines: { bg:"rgba(96,165,250,0.12)",  fg:DARK.accent, bd:"rgba(96,165,250,0.25)",  fillFrom:DARK.accent, fillTo:C.accent },
    Glucides:  { bg:"rgba(34,211,238,0.12)",  fg:"#22D3EE", bd:"rgba(34,211,238,0.25)",  fillFrom:"#22D3EE", fillTo:"#0EA5E9" },
    Lipides:   { bg:"rgba(52,211,153,0.12)",  fg:"#34D399", bd:"rgba(52,211,153,0.25)",  fillFrom:"#34D399", fillTo:C.green },
  };
...
export function CritRow({ crit }) {
  ...
  let palette;
  if (ratio >= 0.8)      palette = { bg:"rgba(52,211,153,0.12)",  bd:"rgba(52,211,153,0.25)",  color:"#34D399" };
  else if (ratio >= 0.5) palette = { bg:"rgba(245,158,11,0.12)",  bd:"rgba(245,158,11,0.25)",  color:"#F59E0B" };
  else                   palette = { bg:"rgba(248,113,113,0.12)", bd:"rgba(248,113,113,0.25)", color:"#F87171" };
