            ...
            {[{id:"journal",l:"Journal"},{id:"bilan",l:"Bilan PRO"}].map(s=>{
              ...
              return(
                <button key={s.id} onClick={()=>{
                  ...
                  border:on?(isBilan?'1px solid rgba(59,130,246,0.35)':'1px solid rgba(0,0,0,0.12)'):'1px solid transparent',
                  ...
                  {isBilan&&!premium&&(
                    <span style={{fontSize:8,marginLeft:4,padding:'1px 4px',borderRadius:3,
                      background:'rgba(59,130,246,0.25)',color:C.blueLt,
          ...
          <>
            ...
            <div style={{padding:'16px 20px 0'}}>
              <div style={{display:'flex',gap:8}}>
                <MacroCard label="Protéines" value={tot.p} goal={pObj} color={DARK.accent} colorDk="#1D4ED8"/>
                ...
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                  <div style={{display:'flex',alignItems:'center',gap:12}}>
                    <div style={{width:40,height:40,borderRadius:12,
                      ...
                      boxShadow:'0 4px 10px rgba(52,211,153,0.35), inset 0 1px 0 rgba(0,0,0,0.12)',
                      position:'relative',overflow:'hidden'}}>
                      <div style={{position:'absolute',inset:0,background:'radial-gradient(110% 60% at 30% 10%,rgba(0,0,0,0.12),transparent 60%)',pointerEvents:'none'}}/>
                  ...
                  <button className="tap" onClick={()=>setEau(e=>Math.min(8,e+1))}
                    style={{padding:'8px 12px',borderRadius:999,background:'transparent',
                      border:'1px solid rgba(52,211,153,0.5)',color:'#34D399',
                    ...
                    return(
                      <button key={i} onClick={()=>setEau(i+1===eau?i:i+1)} className="tap"
                        style={{flex:1,height:24,borderRadius:8,
                          background:on?'linear-gradient(180deg,#34D399,#2DA67D)':'rgba(0,0,0,0.05)',
                          border:`1px solid ${on?'rgba(52,211,153,0.65)':C.bd}`,
                          boxShadow:on?'0 0 8px rgba(52,211,153,0.5), inset 0 1px 0 rgba(0,0,0,0.12)':'none',
                          padding:0}}/>
                    );
                  })}
                </div>
                ...
                <div>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                    ...
                    <div style={{display:'flex',gap:4}}>
                      ...
                      <button onClick={()=>setFruitsV(f=>({...f,legumes:Math.min(10,f.legumes+1)}))} style={{width:24,height:24,borderRadius:8,background:'rgba(52,211,153,0.12)',border:'1px solid rgba(52,211,153,0.25)',color:'#34D399',cursor:'pointer',fontSize:14,display:'grid',placeItems:'center'}}>+</button>
                    </div>
                  </div>
                  <div style={{display:'flex',gap:4}}>
                    {Array.from({length:5}).map((_,i)=>{
                      const on=i<fruitsV.legumes;
                      return <button key={i} onClick={()=>setFruitsV(f=>({...f,legumes:i+1===f.legumes?i:i+1}))} className="tap" style={{flex:1,height:22,borderRadius:8,background:on?'linear-gradient(180deg,#34D399,#2DA67D)':'rgba(0,0,0,0.05)',border:`1px solid ${on?'rgba(52,211,153,0.65)':C.bd}`,boxShadow:on?'0 0 8px rgba(52,211,153,0.35), inset 0 1px 0 rgba(0,0,0,0.12)':'none',padding:0,display:'grid',placeItems:'center',fontSize:10,color:on?'#0B1F18':'transparent'}}>{on?'🥦':''}</button>;
                    })}
                  </div>
                </div>
                {/* Badge objectif atteint */}
                {(fruitsV.fruits+fruitsV.legumes)>=5&&(
                  <div style={{marginTop:12,padding:'8px 12px',background:'rgba(52,211,153,0.08)',border:'1px solid rgba(52,211,153,0.25)',borderRadius:12,fontSize:13,color:'#34D399',fontWeight:600,fontFamily:DISPLAY,textAlign:'center'}}>
            ...
            <div style={{background:C.s1, border:`1px solid ${C.bd}`, borderRadius:16, boxShadow:'0 1px 3px rgba(0,0,0,0.25), inset 0 1px 0 rgba(0,0,0,0.05)', padding:20}}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
                <div style={{width:40,height:40,borderRadius:12,
                  ...
                  boxShadow:'0 4px 10px rgba(59,130,246,0.35), inset 0 1px 0 rgba(0,0,0,0.12)'}}>
                ...
                <button className="tap" onClick={()=>setShowCamera(true)} style={{
                  padding:'8px 16px',background:'rgba(59,130,246,0.12)',
              ...
              {scanRes&&!scanRes.error&&(
                <div style={{padding:16,background:'rgba(52,211,153,0.08)',
                  border:'1px solid rgba(52,211,153,0.25)',borderRadius:16,marginTop:8}}>
                  <div style={{fontWeight:700,fontSize:14,color:'#34D399',marginBottom:12,fontFamily:DISPLAY}}>{scanRes.n}</div>
                  <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:12}}>
                    {[{l:`${scanRes.c} kcal`,c:"#F59E0B"},{l:`P ${scanRes.p}g`,c:DARK.accent},{l:`G ${scanRes.g}g`,c:"#22D3EE"},{l:`L ${scanRes.l}g`,c:"#34D399"}].map(s=>(
              ...
              {scanRes?.error&&(
                <div style={{padding:'12px 12px',background:'rgba(248,113,113,0.12)',
