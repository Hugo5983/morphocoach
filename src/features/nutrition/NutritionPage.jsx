...
function CalorieRing({consumed,goal}){
  ...
  return(
    <div style={{position:'relative',width:224,height:224,margin:'0 auto'}}>
      <div style={{position:'absolute',top:-20,left:-20,width:140,height:140,borderRadius:'50%',background:'radial-gradient(closest-side,rgba(77,139,255,0.20),transparent 70%)',filter:'blur(14px)',pointerEvents:'none'}}/>
      <div style={{position:'absolute',bottom:-20,right:-20,width:160,height:160,borderRadius:'50%',background:'radial-gradient(closest-side,rgba(255,171,93,0.20),transparent 70%)',filter:'blur(14px)',pointerEvents:'none'}}/>
      <svg width="224" height="224" viewBox="0 0 224 224" style={{position:'relative'}}>
        <defs><linearGradient id="brandRing" x1="0" y1="1" x2="1" y2="0">
          <stop offset="0%" stopColor={over?C.coral:C.blue}/><stop offset="50%" stopColor={over?'#FF9580':'#B58BE8'}/><stop offset="100%" stopColor={C.gold}/>
        </linearGradient></defs>
        <g transform="rotate(-90 112 112)">
          <circle cx="112" cy="112" r={r} fill="none" stroke="rgba(190,180,255,0.07)" strokeWidth="9"/>
          <circle cx="112" cy="112" r={r} fill="none" stroke="url(#brandRing)" strokeWidth="9" strokeLinecap="round" strokeDasharray={c} strokeDashoffset={c*(1-pct)} style={{transition:'stroke-dashoffset 1.2s cubic-bezier(.2,.8,.2,1)'}}/>
        </g>
      </svg>
      <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
        <div style={{...eyebrowS}}>{over?'Dépassé':'Restant'}</div>
        <div style={{fontFamily:SERIF,fontSize:60,fontWeight:400,letterSpacing:-2.4,color:over?C.coral:C.text,lineHeight:.95,marginTop:4,...NUM}}>
          {(over?consumed-goal:remaining).toLocaleString('fr-FR').replace(',',' ')}
        </div>
        <div style={{fontSize:11,color:C.mid,fontWeight:500,marginTop:6,letterSpacing:0.2,fontFamily:DISPLAY}}>kcal</div>
      </div>
    </div>
  );
}
...
function MacroCard({label,value,goal,color,colorDk}){
  ...
  return(
    <div style={{flex:1,padding:'14px 12px',borderRadius:16,background:`linear-gradient(160deg,${color}24 0%,${color}08 100%)`,border:`1px solid ${color}30`,boxShadow:`inset 0 1px 0 ${color}30, 0 1px 0 rgba(0,0,0,0.2)`,display:'flex',flexDirection:'column',gap:8,position:'relative',overflow:'hidden'}}>
      <div style={{position:'absolute',top:-20,right:-20,width:60,height:60,borderRadius:'50%',background:`radial-gradient(closest-side,${color}30,transparent 70%)`,pointerEvents:'none'}}/>
          ...
          <>
            ...
            <div style={{padding:'24px 20px 0'}}>
              <div style={{display:'flex',gap:10}}>
                <MacroCard label="Protéines" value={tot.p} goal={pObj} color={C.blue} colorDk={C.blueDk}/>
                <MacroCard label="Glucides" value={tot.g} goal={gObj} color={C.gold} colorDk={C.amberDk}/>
