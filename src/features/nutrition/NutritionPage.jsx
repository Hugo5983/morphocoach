...
// ─── NUTRITION ──────────────────────────────────────────────────────────────
 ...
 <div style={{fontSize:9,letterSpacing:"1.5px",color:"rgba(245,241,232,0.50)",fontWeight:500,marginBottom:6,textTransform:"uppercase"}}>{new Date().toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long"})}</div>
 <div style={{fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:28,fontWeight:300,color:"#F5F1E8",letterSpacing:-1,lineHeight:1.1,marginBottom:2}}>Bonjour, <span style={{fontWeight:500,color:"#4D8BFF"}}>{profil.prenom||"Hugo"}</span></div>
 <div style={{fontSize:11,color:"rgba(245,241,232,0.50)"}}>{obj.l} · Cycle {prog?.numero||1}</div>
 </div>
 ...
 <button key={s.id} onClick={()=>setNView(s.id)} style={{padding:"7px 16px",background:nView===s.id?"rgba(59,130,246,0.08)":"transparent",border:`0.5px solid ${nView===s.id?"#4D8BFF":"rgba(190,180,255,0.07)"}`,borderRadius:20,color:nView===s.id?"#4D8BFF":"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:12,fontWeight:600,whiteSpace:"nowrap",fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",letterSpacing:"0.3px"}}>{s.l}</button>
 ))}
 </div>
 ...
 <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 16px",background:"#141A2E",borderRadius:18,marginBottom:12,border:"0.5px solid rgba(190,180,255,0.07)"}}>
 <Ring pct={calPct} color={tot.cal>calObj?"#FF7A6B":"#4D8BFF"} size={120} stroke={10}>
 <div style={{fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:22,fontWeight:300,color:tot.cal>calObj?C.red:C.text,lineHeight:1,letterSpacing:-1}}>{calLeft}</div>
 <div style={{fontSize:9,color:"rgba(245,241,232,0.50)",marginTop:2}}>kcal restantes</div>
 </Ring>
 <div style={{flex:1,marginLeft:20}}>
 <div style={{marginBottom:10}}>
 <Row style={{justifyContent:"space-between",marginBottom:3}}>
 <span style={{fontSize:11,color:"rgba(245,241,232,0.50)"}}>Consommé</span>
 <span style={{fontSize:12,fontWeight:500,color:tot.cal>calObj?C.red:C.text}}>{tot.cal} kcal</span>
 </Row>
 <Row style={{justifyContent:"space-between",marginBottom:3}}>
 <span style={{fontSize:11,color:"rgba(245,241,232,0.50)"}}>Objectif</span>
 ...
 <div style={{fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:22,fontWeight:300,color:scoreColor,lineHeight:1}}>{scoreLettre}</div>
 <div>
 <div style={{fontSize:10,fontWeight:700,color:scoreColor}}>Score santé</div>
 <div style={{fontSize:9,color:"rgba(245,241,232,0.50)"}}>{score}/100 · Voir détail</div>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* Mini anneaux macros */}
 <div style={{display:"flex",justifyContent:"space-around",padding:"14px 16px",background:"#141A2E",borderRadius:16,marginBottom:12,border:"0.5px solid rgba(190,180,255,0.07)"}}>
 ...
 return(
 <div key={r.id} style={{background:"#141A2E",borderRadius:14,marginBottom:8,border:`0.5px solid ${isActive?'#FFAB5D':'#1e1a10'}`,overflow:"hidden"}}>
 {/* Header repas */}
 <div onClick={()=>setRepasA(isActive?null:r.id)} style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
 <div style={{width:34,height:34,borderRadius:9,background:"#0B0F1F",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0}}>{r.i}</div>
 <div style={{flex:1}}>
 <div style={{fontSize:12,fontWeight:500,color:"#F5F1E8"}}>{r.l}</div>
 <div style={{fontSize:10,color:"rgba(245,241,232,0.50)"}}>{repas[r.id].length>0?`${repas[r.id].length} aliment${repas[r.id].length>1?"s":""}`:"Aucun aliment"}</div>
 {rTot.cal>0&&(
 <div style={{display:"flex",gap:3,marginTop:4}}>
 <div style={{height:2,borderRadius:1,background:"#FFAB5D",flex:rTot.cal,maxWidth:"60%"}}/>
 <div style={{height:2,borderRadius:1,background:"rgba(190,180,255,0.07)",flex:calObj}}/>
 </div>
 )}
 </div>
 <div style={{display:"flex",alignItems:"center",gap:8}}>
 {rTot.cal>0&&<span style={{fontSize:14,fontWeight:300,color:"#FFAB5D"}}>{rTot.cal}</span>}
 <span style={{fontSize:14,color:"rgba(245,241,232,0.32)",transform:isActive?"rotate(180deg)":"none",transition:"transform.2s"}}>⌄</span>
 </div>
 </div>
 {/* Aliments */}
 {isActive&&(
 ...
 <span style={{fontSize:9,color:"rgba(245,241,232,0.50)"}}>kcal</span>
 ...
 <div style={{maxHeight:180,overflowY:"auto",borderRadius:9,border:"0.5px solid rgba(190,180,255,0.07)"}}>
 {filtered.map((item,i)=>(
 ...
 <div><div style={{fontSize:12}}>{item.n}</div><div style={{fontSize:10,color:"rgba(245,241,232,0.50)"}}>{item.c}kcal</div></div>
 ...
 <button key={cat} style={{padding:"4px 10px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:10,whiteSpace:"nowrap",fontFamily:"'Inter',sans-serif"}} onClick={()=>{}}>{cat}</button>
 ))}
 </div>
 <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:6}}>
 {FOODS.filter(f=>!search||f.cat===search).slice(0,8).map((f,i)=>(
 <div key={i} onClick={()=>setRepas(rp=>({...rp,[r.id]:[...rp[r.id],f]}))} style={{padding:"5px 10px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,cursor:"pointer",fontSize:10,color:C.text}}>
 ...
 <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",lineHeight:1.5}}>Basé sur la qualité de vos aliments<br/>et vos comportements nutritionnels</div>
 </div>
 <div style={{textAlign:"center"}}>
 <div style={{fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:48,fontWeight:300,color:scoreColor,lineHeight:1,letterSpacing:-2}}>{scoreLettre}</div>
 <div style={{fontSize:10,color:"rgba(245,241,232,0.50)"}}>{score}/100</div>
 </div>
 </Row>
 ...
 <div style={{marginTop:14,padding:"10px 12px",background:C.s2,borderRadius:9,fontSize:11,color:"rgba(245,241,232,0.50)",lineHeight:1.6}}>
 ...
 <div style={{padding:"9px 11px",background:"rgba(59,130,246,0.08)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:8,fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:12,lineHeight:1.6}}>Base Open Food Facts · 3 millions de produits</div>
 ...
 <button key={r.id} onClick={()=>{setRepas(rp=>({...rp,[r.id]:[...rp[r.id],scanRes]}));setScanRes(null);setScan("");setNView("journal");push("✅","Ajouté !",`${scanRes.n} ajouté au ${r.l.toLowerCase()}.`);}} style={{flex:1,padding:"7px 4px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:7,color:C.text,cursor:"pointer",fontSize:10,fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontWeight:600}}>{r.l}</button>
 ))}
 </div>
 <button onClick={()=>{setMyFoods(f=>[...f,{...scanRes,id:Date.now()}]);setScanRes(null);setScan("");}} style={{marginTop:8,width:"100%",padding:"7px",background:"transparent",border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:7,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:11,fontFamily:"'Inter',sans-serif"}}>💾 Sauvegarder dans ma bibliothèque</button>
 </div>
 )}
 ...
 <button onClick={()=>setRepas(rp=>({...rp,[repasA]:[...rp[repasA],f]}))} style={{padding:"5px 11px",background:"rgba(59,130,246,0.08)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:7,color:C.gold,cursor:"pointer",fontSize:11,fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontWeight:700}}>+</button>
 </Row>
 ))}
 </Box>
 )}
