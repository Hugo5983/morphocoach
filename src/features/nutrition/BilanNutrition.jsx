...
function computeBilan(repasHistory, calObj, pObj, gObj, lObj, profil) {
...
function buildBilanPrompt(bilan, profil, obj, calObj, pObj, gObj, lObj) {
...
function Card({ children, style }) {
  return (
    <div style={{ background:"#111827", border:"1px solid rgba(255,255,255,0.07)",
      borderRadius:16, padding:16, marginBottom:10, ...style }}>
      {children}
    </div>
  );
}
...
function Eyebrow({ children, style }) {
  return (
    <div style={{ fontSize:10, fontWeight:600, letterSpacing:"1.2px",
      ...
      fontFamily:FONT, marginBottom:6, ...style }}>
      {children}
    </div>
  );
}
...
function SectionHeader({ num, title, color = "#3B82F6" }) {
  return (
...
function MacroRow({ label, color, value, goal, pct }) {
...
export default function BilanNutrition({
  ...
  return (
    <div style={{ background:"#0B1220", minHeight:"100vh", paddingBottom:90 }}>
