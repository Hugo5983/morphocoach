...
export default function RepasSheet({
  ...
  const pickFood = (f) => { setQtyFood(f); setAmount(per100Test(f) ? 100 : 1); };
  const closeQty = () => setQtyFood(null);
  const confirmQty = () => {
    if (!qtyFood) return;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: BG, zIndex: 10002,
      ...
      {qtyFood && (() => {
        ...
        const baseName = (qtyFood.n || "").replace(/\s*\(100\s*g\)/i, "").trim();
        return (
          <div onClick={closeQty} style={{ position: "fixed", inset: 0, zIndex: 600,
            background: "rgba(15,25,35,0.45)", backdropFilter: "blur(3px)",
            display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div onClick={e => e.stopPropagation()} className="rs-pop" style={{
              width: "100%", maxWidth: 440, background: BG, borderRadius: "22px 22px 0 0",
              padding: "10px 20px calc(20px + env(safe-area-inset-bottom,0px))",
              boxShadow: "0 -10px 40px rgba(15,25,35,0.2)" }}>
              <div style={{ width: 38, height: 4, borderRadius: 2, background: "rgba(0,0,0,0.12)", margin: "0 auto 16px" }}/>
              <div style={{ fontSize: 17, fontWeight: 700, color: TEXT, fontFamily: SF, marginBottom: 3 }}>{baseName}</div>
