...
export default function Home(props) {
  ...
  const [sleepTarget] = useState(() => parseFloat(localStorage.getItem("morpho_sleep_target") || "8"));
  ...
  return (
    <div className="anim" style={{ padding:"0 16px 28px" }}>
      ...
      {/* ── SOMMEIL (toujours accessible) ─────────────────────── */}
      <Card padding="none" style={{ borderRadius:16, padding:"14px 16px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:42, height:42, borderRadius:12, flexShrink:0,
            background:"linear-gradient(145deg,#818CF8,#6366F1)",
            display:"grid", placeItems:"center",
            boxShadow:"0 4px 12px rgba(99,102,241,0.4)", fontSize:20 }}>😴</div>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:13.5, fontWeight:700, color:C.text, fontFamily:FONT }}>
              Sommeil
            </div>
            <div style={{ fontSize:11.5, color:"#6B7280", marginTop:2, fontFamily:FONT }}>
              {sleepToday != null
                ? `${sleepToday} h cette nuit · cible ${sleepTarget} h`
                : `Note ta nuit · cible ${sleepTarget} h`}
            </div>
          </div>
