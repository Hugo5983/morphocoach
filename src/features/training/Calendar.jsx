        ...
        <div style={{padding:"0 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                {ERGOS.map(e => (
                  <div key={e.id} onClick={() => { setErgo(e); const defs={}; e.params.forEach(p=>{ defs[p.k]=p.def||""; }); setParams(defs); setStep(1); }}
                    style={{padding:"14px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",gap:10,transition:"border-color .15s"}}
          ...
          {step === 1 && ergo && (
            <div>
              ...
              {/* Durée */}
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"14px 16px",marginBottom:10}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <button onClick={() => setDuree(d => Math.max(5,d-5))} style={{width:36,height:36,borderRadius:9,background:C.s2,border:"none",cursor:"pointer",fontSize:18,fontWeight:300,color:"rgba(245,241,232,0.50)"}}>−</button>
                  ...
                  <button onClick={() => setDuree(d => Math.min(180,d+5))} style={{width:36,height:36,borderRadius:9,background:C.accent,border:"none",cursor:"pointer",fontSize:18,color:"#141A2E"}}>+</button>
                </div>
                      ...
                      {p.opts ? (
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:4}}>
                          <input
                            ...
                            style={{flex:1,padding:"8px 10px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:12,color:"#F5F1E8",fontFamily:"'Inter',sans-serif"}}
                          />
              ...
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"14px 16px",marginBottom:10}}>
                ...
                <div style={{marginTop:10,display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <input value={bpm} onChange={e=>setBpm(e.target.value)} placeholder="ex: 145 (facultatif)" style={{flex:1,padding:"7px 10px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:12,color:"#F5F1E8",fontFamily:"'Inter',sans-serif"}}/>
                </div>
              </div>

              {/* Calories */}
              <div style={{background:C.s1,border:`1px solid ${kcalFinal>0?"rgba(59,130,246,0.2)":"rgba(190,180,255,0.07)"}`,borderRadius:12,padding:"14px 16px",marginBottom:16}}>
                ...
                {editKcal ? (
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <input value={kcalManuel} onChange={e=>setKcalManuel(e.target.value)} style={{flex:1,padding:"10px 12px",background:C.s2,border:"0.5px solid #4D8BFF",borderRadius:9,fontSize:16,fontWeight:500,color:"#F5F1E8",fontFamily:"'Outfit','DM Sans',system-ui,sans-serif"}}/>
              ...
              <button onClick={handleSave} style={{width:"100%",padding:"14px",background:C.accent,border:"none",borderRadius:12,color:"#141A2E",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",marginBottom:8}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
                {SPORTS.map(s => (
                  <div key={s.id} onClick={() => setSport(s)}
                    style={{padding:"11px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:11,cursor:"pointer",display:"flex",alignItems:"center",gap:9,transition:"border-color .15s"}}
            ...
            <div>
              ...
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"14px 16px",marginBottom:10}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <button onClick={() => setDuree(d => Math.min(240,d+5))} style={{width:36,height:36,borderRadius:9,background:C.accent,border:"none",cursor:"pointer",fontSize:18,color:"#141A2E"}}>+</button>
                </div>
                <div style={{display:"flex",gap:6,marginTop:12}}>
              ...
              {/* Calories */}
              <div style={{background:C.s1,border:`1px solid ${kcalAuto>0?"rgba(59,130,246,0.2)":"rgba(190,180,255,0.07)"}`,borderRadius:12,padding:"14px 16px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:80}}>
        <div style={{padding:"20px 16px 0",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <button onClick={onClose} style={{background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:10,width:36,height:36,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:18,flexShrink:0,marginLeft:12}}>×</button>
        </div>
        <div style={{padding:"12px 16px",display:"flex",gap:7,flexWrap:"wrap"}}>
          {[{l:"Séries",v:exSerie?.series||exData.s},{l:"Reps",v:exSerie?.reps||exData.r},{l:"Repos",v:exSerie?.repos||exData.rest},{l:"Charge",v:exSerie?.charge||exData.ch}].map(s => (
            <div key={s.l} style={{padding:"8px 10px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:10,textAlign:"center",flex:1,minWidth:60}}>
        ...
        <div style={{padding:"0 16px"}}>
          {tab==="tips" && (
            <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"14px 16px"}}>
          ...
          {tab==="variantes" && (
            <div>
              {(exData.variantes||[]).map((v,i) => (
                <div key={i} style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"14px 16px",marginBottom:8}}>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{padding:"20px 16px",paddingBottom:80}}>
        ...
        {!dbEx && (
          <div style={{padding:"16px 14px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,textAlign:"center"}}>
...
function ExerciceEditable({ ex, exIdx, jourIdx, prog, setProg, cc, METHODS, onGuide, onInfo, checkedEx, toggleCheck, seanceId }) {
  ...
  return (
    <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:10,marginBottom:7,overflow:"hidden",opacity:isChecked?0.6:1,transition:"opacity .15s"}}>
      ...
      {editing && (
       <div style={{marginTop:10,paddingTop:10,borderTop:"0.5px solid rgba(190,180,255,0.07)"}}>
         ...
         {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
          <div key={pp.k}>
           ...
           <div style={{display:"flex",gap:3,alignItems:"center"}}>
            <button onClick={()=>{const c=parseFloat(ex[pp.k])||0;updateEx(pp.k,String(pp.k==="repos"?Math.max(0,c-15):Math.max(1,c-1)));}} style={{width:22,height:22,borderRadius:5,background:C.s2,border:"none",cursor:"pointer",fontSize:12}}>−</button>
            <input value={ex[pp.k]||""} onChange={e=>updateEx(pp.k,e.target.value)} style={{flex:1,padding:"4px 5px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:6,fontSize:11,textAlign:"center",fontFamily:"'Inter',sans-serif"}}/>
            <button onClick={()=>{const c=parseFloat(ex[pp.k])||0;updateEx(pp.k,String(pp.k==="repos"?c+15:c+1));}} style={{width:22,height:22,borderRadius:5,background:C.accent,border:"none",color:"#141A2E",cursor:"pointer",fontSize:12}}>+</button>
           </div>
          </div>
         ))}
        </div>
        <div style={{marginBottom:6}}>
         ...
         <input value={ex.tempo||""} onChange={e=>updateEx("tempo",e.target.value)} placeholder="Ex: 2-1-3" style={{width:"100%",padding:"7px 10px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:11,fontFamily:"'Inter',sans-serif",boxSizing:"border-box"}}/>
        </div>
        <div>
