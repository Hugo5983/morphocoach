...
import { Box, Btn } from "../../components/ui/index.jsx";
              ...
              <>
                ...
                <div onClick={() => setViewSeance(todaySeance)} style={{position:"relative",borderRadius:24,overflow:"hidden",marginBottom:12,padding:"20px 20px 18px",cursor:"pointer",background:`linear-gradient(155deg, ${ic} 0%, ${ic}cc 55%, ${ic}66 100%)`,border:"1px solid rgba(255,255,255,0.22)",boxShadow:`0 20px 40px ${ic}40, inset 0 1px 0 rgba(255,255,255,0.3)`}}>
                  ...
                  <div style={{position:"relative",marginTop:18}}>
                    ...
                    <div style={{fontSize:13,color:"rgba(11,15,31,0.78)",fontWeight:600,marginTop:6,fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif"}}>{INT[todaySeance.intensite||"modere"].l} · {todaySeance.duree}</div>
                  </div>
                </div>

                {/* Checkbox list */}
                {!todaySeance.complete && (
                  <div style={{background:"#141A2E",border:"1px solid rgba(190,180,255,0.07)",borderRadius:18,padding:"6px 16px",marginBottom:12,boxShadow:"inset 0 1px 0 rgba(255,255,255,0.03)"}}>
                    {(todaySeance.exercices||[]).map((ex,idx) => {
                      ...
                      return (
                        <div key={idx} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:last?"none":"1px solid rgba(190,180,255,0.07)"}}>
                          <div onClick={() => toggleCheck(todaySeance.id,idx,ex.repos,todaySeance._calKey)} style={{width:38,height:38,borderRadius:11,flexShrink:0,background:isChecked?"linear-gradient(145deg, #5FE0A5, #2DA67D)":`linear-gradient(145deg, ${cc}30, ${cc}08)`,border:isChecked?"none":`1px solid ${cc}40`,color:isChecked?"#0B1F18":cc,display:"grid",placeItems:"center",cursor:"pointer",fontSize:15,fontWeight:800,fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",boxShadow:isChecked?"0 4px 10px rgba(95,224,165,0.4)":"none"}}>{isChecked?"✓":idx+1}</div>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:14,fontWeight:700,fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",letterSpacing:-0.1,color:isChecked?"rgba(245,241,232,0.32)":"#F5F1E8",textDecoration:isChecked?"line-through":"none"}}>{ex.nom}</div>
                ...
                {!todaySeance.complete && (
                  <button onClick={() => setViewSeance(todaySeance)} style={{width:"100%",padding:"16px 20px",borderRadius:18,background:"#F5F1E8",color:"#0B0F1F",border:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:15,fontWeight:700,letterSpacing:0.2,cursor:"pointer",boxShadow:"0 10px 24px rgba(245,241,232,0.16), inset 0 1px 0 rgba(255,255,255,0.6)"}}>
                    ▶ Démarrer la séance
                  </button>
                )}
