...
// Structure scroll simple : div fixe plein écran → overflow-y auto. Pas de
// swipe-back (source du bug scroll). Bouton retour en haut.
...
export default function ProgressionPage({ EX, prog, setProg, push, onClose }) {
  ...
  useEffect(() => {
    ...
    return () => { document.documentElement.style.overflow = ""; document.body.style.overflow = ""; window.scrollTo(0, y); };
  }, []);

  // Build muscle groups from programme
  const muscles = useMemo(() => {
    const map = {};
    (prog?.jours || []).forEach(j => (j.exercices || []).forEach(ex => {
      const g = ex.groupe || "Autre";
      if (!map[g]) map[g] = [];
      if (!map[g].find(e => e.nom === ex.nom)) map[g].push(ex);
    }));
    return Object.entries(map).map(([id, exs]) => ({ id, exs }));
  }, [prog]);

  // Auto-select first muscle
  useEffect(() => { if (muscles.length && !selMuscle) setSelMuscle(muscles[0].id); }, [muscles]);

  // Exercises for selected muscle
  const exos = useMemo(() => {
    const grp = muscles.find(m => m.id === selMuscle);
    if (!grp) return [];
    return grp.exs.map(ex => {
      let hist = [...(ex.historique || [])];
      const rec = prog?.records?.[ex.nom];
      if (Array.isArray(rec)) rec.forEach(h => hist.push(h));
      else if (rec?.historique) rec.historique.forEach(h => hist.push(h));
      const seen = new Set();
      hist = hist.filter(h => { const k = `${h.date}-${h.poids}-${h.reps}`; if (seen.has(k)) return false; seen.add(k); return true; })
        .sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
      const pts = hist.map(h => ({ rm: calc1RM(parseFloat(h.poids), parseInt(h.reps)), date: h.date, poids: parseFloat(h.poids), reps: parseInt(h.reps) })).filter(d => d.rm > 0);
      ...
      const pct = firstRM > 0 ? Math.round(((bestRM - firstRM) / firstRM) * 100) : 0;
      return { nom: ex.nom, equip: ex.equipement || "", pts, bestRM, firstRM, lastRM, pct };
    });
  }, [muscles, selMuscle, prog]);
  ...
  const fmtD  = d => { if (!d) return "—"; const p = d.split(/[-/]/); return p.length >= 2 ? `${p[p.length-2]}/${p[p.length-3]||p[0]}` : d; };
  ...
  const sp = (hi - lo) || 1;
  const pts = chart.map((s, i) => ({ x: PL + (chart.length === 1 ? cW / 2 : (i / (chart.length - 1)) * cW), y: PT + cH - ((s.rm - lo) / sp) * cH, ...s }));
  ...
  return (
    <div ref={scrollRef} style={{
      ...
      <div style={{ padding: "20px 18px 160px", maxWidth: 500, margin: "0 auto" }}>
            ...
            <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:4 }}>
              {muscles.map(m => (
                <button key={m.id} onClick={() => { setSelMuscle(m.id); setSelExIdx(0); }} style={{
                  ...
                  fontSize:13, fontWeight: m.id === selMuscle ? 700 : 600, fontFamily:F, cursor:"pointer",
            ...
            <div style={{ display:"flex", gap:10, overflowX:"auto", padding:"2px 2px 4px" }}>
              {exos.map((ex, i) => (
                <div key={ex.nom} onClick={() => setSelExIdx(i)} style={{
                  ...
                  <div style={{ fontSize:12, fontWeight:600, color:"#9AA3B2", marginTop:5, fontFamily:F }}>{ex.equip || "—"}</div>
                  {ex.bestRM > 0
          ...
          <div style={{ position:"relative", borderRadius:26, overflow:"hidden", background:"#0B0F1F", marginBottom:16,
            ...
            <div style={{position:"relative",padding:"20px 20px 22px"}}>
              ...
              <div style={{display:"flex",alignItems:"baseline",gap:8}}>
                <span style={{fontSize:66,fontWeight:700,letterSpacing:"-0.05em",lineHeight:0.9,color:"#fff",fontVariantNumeric:"tabular-nums",fontFamily:F}}>{Math.round(best)}</span>
            ...
            <div style={{position:"relative",padding:"26px 4px 4px",marginBottom:16}}>
              <div style={{position:"relative",height:10,borderRadius:99,background:"#EEF0F5"}}>
                <div style={{position:"absolute",top:0,left:0,bottom:0,borderRadius:99,
                  background:`linear-gradient(90deg,${BL},${GRN})`,width:`${objPct}%`,boxShadow:`0 2px 8px rgba(16,185,129,0.35)`}}/>
        ...
        {sel && hasData && (
          <div style={{display:"flex",gap:10,marginBottom:16,animation:"prUp .5s cubic-bezier(.22,1,.36,1) .21s both"}}>
            {[{l:"DÉPART",d:"#C6CEDE",lc:"#9AA3B2",v:first,bc:"rgba(15,25,35,0.06)",bs:"rgba(15,25,35,0.04)"},
              {l:"RECORD",d:"#F5A623",lc:"#C77E12",v:best,bc:"rgba(245,158,11,0.22)",bs:"rgba(245,158,11,0.08)"},
              {l:"ACTUEL",d:GRN,lc:"#059669",v:last,bc:`rgba(16,185,129,0.22)`,bs:"rgba(16,185,129,0.08)"}
            ].map(s=>(
              <div key={s.l} style={{flex:1,background:"#fff",border:`1px solid ${s.bc}`,borderRadius:18,padding:"14px 12px",boxShadow:`0 2px 10px ${s.bs}`}}>
        ...
        {sel && chart.length > 0 && (
          <div style={{background:"#fff",border:"1px solid rgba(15,25,35,0.06)",borderRadius:22,padding:"16px 18px",marginBottom:16,
            boxShadow:"0 2px 12px rgba(15,25,35,0.05)",animation:"prUp .5s cubic-bezier(.22,1,.36,1) .24s both"}}>
              ...
              {chart.slice(-5).reverse().map((s,i)=>{
                ...
                return <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 0",borderTop:"1px solid rgba(15,25,35,0.05)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:9}}>
                    ...
                    {isBest&&<span style={{fontSize:9.5,fontWeight:800,color:"#C77E12",background:"rgba(245,158,11,0.14)",borderRadius:6,padding:"2px 6px",letterSpacing:"0.05em",fontFamily:F}}>PR</span>}
                  </div>
