    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:32}}>
        <div style={{padding:"20px 16px 0",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <button onClick={onClose} style={{background:C.s2,border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,width:36,height:36,color:C.mid,cursor:"pointer",fontSize:20,flexShrink:0,marginLeft:12}}>×</button>
        </div>
        <div style={{padding:"12px 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
          {[{l:"Séries",v:exSerie?.series||exData.s},{l:"Reps",v:exSerie?.reps||exData.r},{l:"Repos",v:exSerie?.repos||exData.rest},{l:"Charge",v:exSerie?.charge||exData.ch}].map(s=>(
            <div key={s.l} style={{padding:"8px 12px",background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,textAlign:"center",flex:1,minWidth:60}}>
        ...
        <div style={{padding:"0 16px"}}>
          {tab==="tips"&&(<div style={{background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,padding:"16px 16px"}}>
            <div style={{fontSize:10,color:C.mid,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Conseils techniques</div>
            {(exData.tips||[]).map((tip,i)=>(<div key={i} style={{display:"flex",gap:12,marginBottom:16,paddingBottom:16,borderBottom:i<(exData.tips||[]).length-1?"0.5px solid rgba(190,180,255,0.07)":"none"}}><div style={{width:22,height:22,borderRadius:"50%",background:"rgba(59,130,246,0.1)",border:"0.5px solid rgba(59,130,246,0.2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10,fontWeight:500,color:"#4D8BFF"}}>{i+1}</div><div style={{fontSize:13,color:"${C.text}",lineHeight:1.6}}>{tip}</div></div>))}
            {exData.prog&&<div style={{marginTop:4,padding:"12px 12px",background:"rgba(34,197,94,0.08)",border:"0.5px solid rgba(34,197,94,0.2)",borderRadius:8}}><div style={{fontSize:10,color:"#5FE0A5",fontWeight:500,letterSpacing:"1px",textTransform:"uppercase",marginBottom:4}}>Progression</div><div style={{fontSize:13,color:C.mid,lineHeight:1.5}}>{exData.prog}</div></div>}
          </div>)}
          {tab==="variantes"&&(<div>{(exData.variantes||[]).map((v,i)=>(<div key={i} style={{background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,padding:"16px 16px",marginBottom:8}}><div style={{fontSize:13,fontWeight:500,color:"${C.text}",marginBottom:4}}>{v.nom||v}</div>{v.note&&<div style={{fontSize:11,color:C.mid,lineHeight:1.5}}>{v.note}</div>}</div>))}</div>)}
          {tab==="erreurs"&&(<div style={{background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,padding:"16px 16px"}}>
            <div style={{fontSize:10,color:C.mid,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Erreurs à éviter</div>
            {(exData.erreurs||[]).map((e,i)=>(<div key={i} style={{display:"flex",gap:12,marginBottom:12,alignItems:"flex-start"}}><div style={{width:20,height:20,borderRadius:"50%",background:"rgba(248,113,113,0.1)",border:"0.5px solid rgba(248,113,113,0.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10,color:"#FF7A6B"}}>✕</div><div style={{fontSize:13,color:"${C.text}",lineHeight:1.5}}>{e}</div></div>))}
          </div>)}
          {tab==="morpho"&&(<div style={{background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,padding:"16px 16px"}}>
            <div style={{fontSize:10,color:C.mid,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Adaptation morphologique</div>
            {(exData.morpho||"").split("\n").filter(Boolean).map((line,i,arr)=>(<div key={i} style={{display:"flex",gap:8,marginBottom:12,paddingBottom:12,borderBottom:i<arr.length-1?"0.5px solid rgba(190,180,255,0.07)":"none",alignItems:"flex-start"}}><div style={{fontSize:13,flexShrink:0,marginTop:1}}>{line.split(":")[0].trim()}</div><div style={{fontSize:11,color:"${C.text}",lineHeight:1.6,flex:1}}>{line.split(":").slice(1).join(":").trim()}</div></div>))}
            {!(exData.morpho||"").includes("\n")&&exData.morpho&&<div style={{fontSize:13,color:"${C.text}",lineHeight:1.7}}>{exData.morpho}</div>}
          </div>)}
        </div>
        <div style={{padding:"16px 16px 0"}}>
          <button onClick={onClose} style={{width:"100%",padding:"12px",background:"transparent",border:"0.5px solid rgba(0,0,0,0.06)",borderRadius:12,color:C.mid,cursor:"pointer",fontSize:13,fontFamily:"'Inter',sans-serif"}}>← Retour à la séance</button>
        </div>
      </div>
    </div>
  );
...
export function SeanceDetailModal({ jour, jourIdx, prog, setProg, onClose, C, INT }) {
  ...
  const lbl = {fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",marginBottom:8,fontFamily:DISP_F};
    ...
    return (
      <div style={{minHeight:"100vh",background:C.bg}}>
        <div style={{padding:"20px 16px 32px"}}>
          <button onClick={()=>setNewExForm(null)} style={{background:"transparent",border:"none",color:"#60A5FA",cursor:"pointer",fontSize:14,fontWeight:600,fontFamily:DISP_F,display:"flex",alignItems:"center",gap:4,marginBottom:16}}>
          ...
          {/* Badge exo */}
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:`${cc(newExForm.cat)}0d`,border:`1px solid ${cc(newExForm.cat)}30`,borderRadius:16,marginBottom:20}}>
            ...
            <div>
              ...
              <div style={{fontSize:10,color:C.mid,marginTop:2,fontFamily:DISP_F}}>{newExForm.group}</div>
            </div>
          </div>
          ...
          <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:16,padding:"16px",marginBottom:12}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {METHODS.slice(0,8).map(mm=>{const on=newExForm.methode===mm;return(
                <button key={mm} onClick={()=>setNewExForm(f=>({...f,methode:mm}))} style={{padding:"4px 12px",borderRadius:999,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(59,130,246,0.12)":"transparent",color:on?"#60A5FA":C.mid,cursor:"pointer",fontSize:11,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
              );})}
            </div>
          </div>

          <button onClick={addEx} style={{width:"100%",padding:"16px",background:"linear-gradient(180deg,#3B82F6,#2563EB)",border:"none",borderRadius:16,color:"#FFF",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:DISP_F,boxShadow:"0 8px 24px rgba(59,130,246,0.32)"}}>
            + Ajouter à la séance
          </button>
        </div>
      </div>
  ...
  if (showBiblio) {
    return (
      <div style={{minHeight:"100vh",background:C.bg}}>
        <div style={{padding:"20px 16px 32px"}}>
          {/* Back */}
          <button onClick={()=>{setShowBiblio(false);setSearch("");setGroupe(null);}} style={{background:"transparent",border:"none",color:"#60A5FA",cursor:"pointer",fontSize:14,fontWeight:600,fontFamily:DISP_F,display:"flex",alignItems:"center",gap:4,marginBottom:16}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
            Retour à la séance
          </button>

          <div style={{fontFamily:SERIF_F,fontSize:20,letterSpacing:-0.5,marginBottom:16}}>Ajouter un <span style={{fontStyle:"italic",color:"#60A5FA"}}>exercice</span></div>
          ...
          {exosList.length > 0 && exosList.map((ex, i) => {
            ...
            return (
              <div key={i} style={{background:C.s1,border:`1px solid ${already?"rgba(52,211,153,0.25)":C.bd}`,borderRadius:16,padding:16,marginBottom:12,borderLeft:`3px solid ${col}`,boxShadow:`0 8px 24px -16px ${col}`}}>
                {/* Badge catégorie */}
                <span style={{fontSize:11,fontWeight:700,letterSpacing:"1.2px",padding:"4px 8px",borderRadius:8,display:"inline-block",marginBottom:8,color:col,background:`${col}18`,border:`1px solid ${col}35`,fontFamily:DISP_F}}>
                  {ex.cat?.toUpperCase()}
                </span>
                {/* Nom */}
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  {dbEx && (
                    <button onClick={()=>{const d=findExInDB(ex.nom);if(d)setGuideEx({dbEx:d,serieEx:ex});}}
                      style={{padding:"12px 16px",background:"rgba(59,130,246,0.10)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:12,color:"#60A5FA",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:DISP_F}}>
                      Guide →
                    </button>
                  )}
                </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:32}}>
        ...
        <div style={{padding:"20px 16px 0",display:"flex",alignItems:"center"}}>
          <button onClick={onClose} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(96,165,250,0.10)",border:"1px solid rgba(96,165,250,0.25)",borderRadius:12,padding:"8px 16px",color:"#60A5FA",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:DISP_F}}>
        ...
        <div style={{padding:"16px 16px 0"}}>
          {/* Hero séance */}
          <div style={{position:"relative",borderRadius:28,overflow:"hidden",marginBottom:16,padding:"20px 20px 20px",background:`linear-gradient(155deg,${int.c} 0%,${int.c}cc 55%,${int.c}66 100%)`,border:"1px solid rgba(0,0,0,0.11)",boxShadow:`0 20px 40px ${int.c}40`}}>
            ...
            <div style={{position:"relative"}}>
              <div style={{display:"inline-flex",alignItems:"center",gap:4,padding:"4px 8px",borderRadius:999,background:"rgba(11,15,31,0.28)",color:DARK.bg,fontSize:10,fontWeight:700,fontFamily:DISP_F,letterSpacing:1.4,marginBottom:16}}>
                <span style={{width:6,height:6,borderRadius:"50%",background:DARK.bg}}/>
                {int.l.toUpperCase()}
              </div>
              <div style={{fontFamily:SERIF_F,fontSize:34,fontWeight:400,letterSpacing:-1.6,color:DARK.bg,lineHeight:0.98,marginBottom:8}}>{localName}</div>
              ...
              <div style={{display:"flex",gap:24}}>
                {[{v:jour.duree||"45-60",u:"min"},{v:exercices.length,u:"exercices"}].map(s=>(
                  <div key={s.u}>
                    <div style={{fontFamily:DISP_F,fontSize:20,fontWeight:700,color:DARK.bg,letterSpacing:-0.4}}>{s.v}</div>
                    <div style={{fontSize:10,color:"rgba(11,15,31,0.68)",fontWeight:700,fontFamily:DISP_F,letterSpacing:0.4,marginTop:2}}>{s.u.toUpperCase()}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:16,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",marginBottom:4,fontFamily:DISP_F}}>Nom de la séance</div>
              <input value={localName} onChange={e=>updateName(e.target.value)}
                autoComplete="off" autoCorrect="off" data-form-type="other"
                style={{width:"100%",background:"none",border:"none",color:"${C.text}",fontFamily:DISP_F,fontSize:16,fontWeight:600,outline:"none",padding:0}}/>
            </div>
          </div>

          {/* Exercices */}
          {exercices.length === 0 && (
            <div style={{borderRadius:20,padding:"20px 16px",background:"rgba(0,0,0,0.01)",border:`1px dashed ${C.bd}`,marginBottom:16}}>
              {[{w:"65%",col:"#4D8BFF"},{w:"50%",col:"#5FE0A5"},{w:"72%",col:"#B69DFF"}].map((g,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:i<2?`1px solid rgba(0,0,0,0.03)`:"none",opacity:1-i*0.2}}>
                  <div style={{width:34,height:34,borderRadius:12,background:`${g.col}15`,border:`1px solid ${g.col}25`,flexShrink:0}}/>
                  <div style={{flex:1}}>
                    <div style={{height:11,borderRadius:4,background:"rgba(0,0,0,0.05)",width:g.w,marginBottom:8}}/>
              ...
              <div key={k} style={{background:C.s1,border:`1px solid ${isEditing?colour+"40":C.bd}`,borderRadius:16,marginBottom:8,overflow:"hidden"}}>
                <div style={{padding:"12px 16px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:38,height:38,borderRadius:12,flexShrink:0,background:`linear-gradient(145deg,${colour}30,${colour}08)`,border:`1px solid ${colour}40`,color:colour,display:"grid",placeItems:"center",fontFamily:DISP_F,fontSize:13,fontWeight:700}}>{k+1}</div>
                    ...
                    <div style={{display:"flex",gap:4,marginLeft:8}}>
                      {findExInDB(ex.nom) && (
                        <button onClick={()=>{const d=findExInDB(ex.nom);if(d)setGuideEx({dbEx:d,serieEx:ex});}} style={{padding:"4px 8px",background:"rgba(59,130,246,0.06)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:8,color:"#60A5FA",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:DISP_F}}>Guide›</button>
                      )}
                      <button onClick={()=>setEditEx(m=>({...m,[k]:!m[k]}))} style={{width:30,height:30,borderRadius:8,background:"rgba(59,130,246,0.08)",border:"0.5px solid rgba(59,130,246,0.2)",color:"#60A5FA",cursor:"pointer",fontSize:13,display:"grid",placeItems:"center"}}>✏️</button>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                        {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
                          <div key={pp.k} style={{minWidth:0}}>
                            <div style={{fontSize:8,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",marginBottom:4,fontFamily:DISP_F}}>{pp.l}</div>
                            <div style={{display:"flex",gap:4,alignItems:"center"}}>
                              ...
                              <button onClick={()=>{const cur=parseFloat(ex[pp.k])||0;updateEx(k,pp.k,String(cur+(pp.k==="repos"?15:1)));}} style={{width:24,height:24,borderRadius:8,background:"rgba(59,130,246,0.16)",border:"none",cursor:"pointer",fontSize:14,color:"#60A5FA",flexShrink:0,display:"grid",placeItems:"center"}}>+</button>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",marginBottom:8,fontFamily:DISP_F}}>Méthode</div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {METHODS.map(mm=>{const on=ex.methode===mm;return(
                          <button key={mm} onClick={()=>updateEx(k,"methode",mm)} style={{padding:"4px 12px",borderRadius:999,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(59,130,246,0.12)":"transparent",color:on?"#60A5FA":C.mid,cursor:"pointer",fontSize:10,fontFamily:DISP_F}}>{mm}</button>
                        );})}
                      </div>
                    </div>
                  )}
          ...
          <button onClick={()=>setShowBiblio(true)} style={{width:"100%",padding:"16px",background:"rgba(59,130,246,0.06)",border:"1px dashed rgba(59,130,246,0.5)",borderRadius:16,color:"#60A5FA",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:DISP_F,display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:4}}>
