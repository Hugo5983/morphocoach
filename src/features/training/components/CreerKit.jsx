...
.mc-empty{text-align:center;padding:32px 12px;color:${MID};font-size:14px;
  ...
  border:1px dashed ${BD};border-radius:20px;background:rgba(255,255,255,.02)}
`;
    ...
    <div className="mc-ex mc-pop" style={{ borderLeft:`3px solid ${catColor}`,
      ...
      <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
        <div style={{ flex:1, minWidth:0 }}>
          <span style={{ fontSize:11, fontWeight:700, letterSpacing:"1.2px", padding:"4px 8px",
            borderRadius:8, display:"inline-block", marginBottom:8,
            color:catColor, background:`${catColor}18`, border:`1px solid ${catColor}35` }}>
            {catLabel}
          </span>
          <div style={{ fontSize:16, fontWeight:700, color:TEXT, fontFamily:F, letterSpacing:-.3, lineHeight:1.2, marginBottom:4 }}>{ex.nom}</div>
      ...
      {open && (
        <div className="mc-adv">
          <div>
            <div style={{ fontSize:10, fontWeight:700, letterSpacing:".9px", textTransform:"uppercase",
  ...
  if (guideEx) return (
    <div style={{ position:"fixed", inset:0, background:BG, zIndex:340, overflowY:"auto" }}>
      <div style={{ maxWidth:500, margin:"0 auto", padding:"20px 20px 32px" }}>
        ...
        <div style={{ fontSize:20, fontWeight:700, color:TEXT, fontFamily:F, marginBottom:16, letterSpacing:-.4 }}>{guideEx.n}</div>
        ...
        <div style={{ padding:"12px 0" }}>
          {guideTab==="tips" && (guideEx.tips||[]).map((tip,i,arr) => (
            <div key={i} style={{ display:"flex", gap:12, paddingBottom:16, marginBottom:16,
              ...
              <div style={{ fontSize:13, color:TEXT, lineHeight:1.7 }}>{tip}</div>
            </div>
          ))}
          ...
          {list.map((ex,i) => {
            ...
            return (
              <div key={i} className={`mc-lib${added?" added":""}`}>
                ...
                <div style={{ fontSize:16, fontWeight:700, color:TEXT, marginBottom:4, fontFamily:F, letterSpacing:-.2 }}>{ex.n}</div>
