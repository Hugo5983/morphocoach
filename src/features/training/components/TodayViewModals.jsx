...
export function ManualRMModal({ prog, setProg, onClose, onSelectExercise, push, C }) {
  ...
  const groupeColor = (g) => {
    const map = {
      Pectoraux:"#3B82F6", Dos:"#34D399", Epaules:"#818CF8",
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(15,25,35,0.5)", display:"flex", alignItems:"flex-end", justifyContent:"center" }}
      ...
      <div style={{ width:"100%", maxWidth:480, background:"#fff", borderRadius:"22px 22px 0 0", maxHeight:"92vh", overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
        ...
        <div style={{ padding:"0 20px" }}>
          ...
          <div style={{ display:"flex", gap:7, overflowX:"auto", paddingBottom:12, scrollbarWidth:"none" }}>
            <button onClick={() => { setGroupe(null); setSearch(""); }}
              style={{ flexShrink:0, padding:"7px 14px", borderRadius:99, border:("1.5px solid " + (!groupe && !search ? "#0F1923" : "rgba(0,0,0,0.08)")), background:(!groupe && !search ? "#0F1923" : "#fff"), color:(!groupe && !search ? "#fff" : "#374151"), fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:DISP_F }}>
              Tous
            </button>
            {groupes.map(g => (
              <button key={g} onClick={() => { setGroupe(g === groupe ? null : g); setSearch(""); }}
                style={{ flexShrink:0, padding:"7px 14px", borderRadius:99, border:("1.5px solid " + (groupe === g ? groupeColor(g) : "rgba(0,0,0,0.08)")), background:(groupe === g ? groupeColor(g) : "#fff"), color:(groupe === g ? "#fff" : "#374151"), fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:DISP_F, whiteSpace:"nowrap" }}>
                {g}
              </button>
            ))}
          </div>
            ...
            {exosList.map((ex, i) => {
              ...
              return (
                <div key={i} onClick={() => handleSelect(ex)}
                  style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 15px", borderBottom: notLast ? "1px solid rgba(0,0,0,0.04)" : "none", cursor:"pointer" }}>
                  <div style={{ width:9, height:9, borderRadius:"50%", background:col, flexShrink:0 }}/>
                  <div style={{ flex:1, fontSize:13.5, fontWeight:600, color:C.text, fontFamily:DISP_F, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                    {ex.nom}
                  </div>
                  {bestRM && (
