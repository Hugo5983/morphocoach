...
import { calc1RM, calcKgFor, findExInDB } from "../../../utils/training.js";
...
export function ManualRMModal({ prog, setProg, onClose, push, C }) {
  ...
  // Couleurs par groupe musculaire (cohérentes avec les dots Photo 2)
  const groupeColor = (g) => ({
    Pectoraux:"#3B82F6", Dos:"#34D399", Épaules:"#818CF8",
    Jambes:"#F59E0B", Bras:"#FB923C", Gainage:"#06B6D4",
  }[g] || "#9CA3AF");

  const catColor = (cat) => ({
    principal:"#3B82F6", correctif:"#F87171", gainage:"#34D399", isolation:"#818CF8"
  }[cat||"principal"] || "#3B82F6");

  // Trouver le meilleur 1RM déjà enregistré pour un exercice
  const getBestRM = (nomEx) => {
    let best = 0;
    // Chercher dans les séances
    ...
    // Chercher dans records standalone
    const rec = prog?.records?.[nomEx];
    if (Array.isArray(rec)) rec.forEach(h => {
      const rm = calc1RM(parseFloat(h.poids), parseInt(h.reps));
      if (rm > best) best = rm;
    });
  ...
  // Liste exercices (toujours affichée, filtrée par groupe ou recherche)
  ...
  const exosList = useMemo(() => {
    const base = search
      ...
      : groupe
        ? allExos.filter(e => e.group === groupe)
        : allExos;
    return base;
  ...
  const handleSave = () => {
    ...
    if (!found) { u.records = u.records||{}; u.records[selected.nom] = u.records[selected.nom]||{}; u.records[selected.nom].historique = [...(u.records[selected.nom].historique||[]), entry]; }
    setProg(u);
  ...
  const sheetStyle   = {...SHEET_BASE, maxHeight:"92vh", overflowY:"auto", WebkitOverflowScrolling:"touch"};
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 20px 14px"}}>
          <div>
            ...
            <div style={{fontFamily:SERIF_F,fontSize:22,fontWeight:700,color:C.text,letterSpacing:-0.5}}>
              {selected ? selected.nom : <>Ajouter un <span style={{fontStyle:"italic",color:"#60A5FA"}}>exercice</span></>}
            </div>
          </div>
            ...
            <div>
              ...
              <div style={{position:"relative",marginBottom:12}}>
                ...
                <input value={search} onChange={e=>{setSearch(e.target.value);setGroupe(null);}}
                  ...
                  style={{width:"100%",padding:"12px 16px 12px 38px",background:"#F6F8FB",border:"1px solid rgba(0,0,0,0.06)",borderRadius:14,fontSize:14,color:C.text,fontFamily:DISP_F,boxSizing:"border-box",outline:"none"}}
                />
              </div>

              {/* Filtres groupe — chips compacts */}
              <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:12,scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
                <button onClick={()=>{setGroupe(null);setSearch("");}}
                  style={{flexShrink:0,padding:"7px 14px",borderRadius:99,border:`1.5px solid ${!groupe&&!search?"#0F1923":"rgba(0,0,0,0.08)"}`,background:!groupe&&!search?"#0F1923":"#fff",color:!groupe&&!search?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:DISP_F,transition:"all .15s"}}>
                  Tous
                </button>
                {groupes.map(g => {
                  const on = groupe===g;
                  return (
                    <button key={g} onClick={()=>{setGroupe(g===groupe?null:g);setSearch("");}}
                      style={{flexShrink:0,padding:"7px 14px",borderRadius:99,border:`1.5px solid ${on?groupeColor(g):"rgba(0,0,0,0.08)"}`,background:on?groupeColor(g):"#fff",color:on?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:DISP_F,transition:"all .15s",whiteSpace:"nowrap"}}>
                      {g} <span style={{opacity:.6,fontWeight:500,fontSize:10}}>({(EX[g]||[]).length})</span>
                    </button>
                  );
                })}
              </div>

              {/* Liste compacte — dot + nom + record existant + flèche */}
              <div style={{background:C.s1,border:"1px solid rgba(0,0,0,0.06)",borderRadius:16,overflow:"hidden",marginBottom:16}}>
                {exosList.length === 0 && (
                  <div style={{padding:"20px",textAlign:"center",fontSize:13,color:"#9CA3AF",fontFamily:DISP_F}}>
                    {search ? `Aucun résultat pour « ${search} »` : "Sélectionne un groupe musculaire"}
                  </div>
                )}
                {exosList.map((ex, i) => {
                  const bestRM = getBestRM(ex.nom);
                  const col    = groupeColor(ex.group);
                  const isLast = i === exosList.length - 1;
                  return (
                    <div key={i} onClick={()=>setSelected(ex)}
                      ...
                      {/* Nom */}
                      <div style={{flex:1,fontSize:13.5,fontWeight:600,color:C.text,fontFamily:DISP_F,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                        {ex.nom}
                      </div>
                      {/* Record existant (si disponible) */}
                      {bestRM && (
                        <div style={{fontSize:13,fontWeight:700,color:"#60A5FA",fontFamily:DISP_F,flexShrink:0,fontVariantNumeric:"tabular-nums"}}>
                          {bestRM} kg
                        </div>
                      )}
                      {/* Flèche */}
