...
export function ManualRMModal({ prog, setProg, onClose, push, C }) {
  ...
  // Guide exercice en plein écran (réutilise le composant existant)
  if (guideEx) {
    return (
            ...
            <div>
              ...
              <div style={{position:"relative",marginBottom:12}}>
                <input value={search} onChange={e=>{setSearch(e.target.value);setGroupe(null);}}
                  ...
                  style={{width:"100%",padding:"12px 16px 12px 40px",background:C.s1,border:"1px solid rgba(0,0,0,0.06)",borderRadius:14,fontSize:14,color:C.text,fontFamily:DISP_F,boxSizing:"border-box",outline:"none"}}
                />
                <div style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:14,color:C.dim,pointerEvents:"none"}}>🔍</div>
              </div>

              {/* Groupes musculaires — pills horizontales (style photo 3) */}
              {!search && (
                  ...
                  {groupes.map(g => {
                    ...
                    return (
                      <button key={g} onClick={() => setGroupe(g===groupe?null:g)}
                        style={{flexShrink:0,padding:"9px 16px",borderRadius:12,border:`1.5px solid ${on?"#3B82F6":C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#3B82F6":"#374151",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:DISP_F,whiteSpace:"nowrap",transition:"all .18s"}}>
              ...
              {/* Cartes riches exercices (style photo 3) */}
              {exosList.length > 0 && exosList.map((ex, i) => {
                const col = cc(ex.cat);
                const dbEx = ex.raw;
                return (
                  <div key={i} style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:16,padding:16,marginBottom:10,borderLeft:`3px solid ${col}`,boxShadow:`0 8px 24px -16px ${col}`}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <button onClick={()=>setSelected(ex)}
                        style={{flex:1,padding:"13px",borderRadius:13,border:"none",fontFamily:DISP_F,fontSize:13,fontWeight:700,cursor:"pointer",
                          background:"linear-gradient(180deg,#3B82F6,#2563EB)",color:"#fff",boxShadow:"0 6px 18px rgba(59,130,246,0.28)"}}>
                        + Choisir
                      </button>
                      {dbEx && (
                        <button onClick={()=>{const d=findExInDB(ex.nom)||dbEx;if(d)setGuideEx(d);}}
              ...
              {search && exosList.length === 0 && (
                <div style={{textAlign:"center",padding:"20px 0",fontSize:12,color:C.dim,fontFamily:DISP_F}}>Aucun exercice trouvé pour « {search} »</div>
              )}
            </div>
          ) : (
            <div>
