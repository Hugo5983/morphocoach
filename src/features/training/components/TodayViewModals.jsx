...
const SHEET_BASE = {
  ...
  background:C.s1, border:"1px solid rgba(60,91,255,0.35)",
...
export const OBJ_TARGET = {
  hypertrophie:    { reps:10, pct:75, l:"Hypertrophie",    color:"#3C5BFF", desc:"10 reps · Volume musculaire"      },
  force:           { reps:5,  pct:87, l:"Force",            color:"#3C5BFF", desc:"5 reps · Force maximale"          },
  poids:           { reps:15, pct:64, l:"Perte de poids",   color:"#12B76A", desc:"15 reps · Brûle des calories"     },
  sante:           { reps:12, pct:70, l:"Santé générale",   color:"#12B76A", desc:"12 reps · Maintien & tonicité"    },
  prep_physique:   { reps:8,  pct:80, l:"Prépa physique",   color:"#F59E0B", desc:"8 reps · Puissance & condition"   },
  reathletisation: { reps:15, pct:64, l:"Réathlétisation",  color:"#3C5BFF", desc:"15 reps · Progression douce"      },
};
export const DEFAULT_TARGET = { reps:10, pct:75, l:"Hypertrophie", color:"#3C5BFF", desc:"10 reps · Volume musculaire" };
...
export function ManualRMModal({ prog, setProg, onClose, onSelectExercise, push, C }) {
  ...
  const groupeColor = (g) => {
    const map = {
      Pectoraux:C.accent, Dos:"#12B76A", Epaules:"#9DB0FF",
      Jambes:"#F59E0B", Bras:"#3C5BFF", Gainage:"#3C5BFF",
    };
    return map[g] ||"#98A2B3";
  };
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(16,19,24,0.5)", display:"flex", alignItems:"flex-end", justifyContent:"center" }}
      ...
      <div style={{ width:"100%", maxWidth:480, background:"#FFF", borderRadius:"20px 20px 0 0", maxHeight:"92vh", overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
        ...
        <div style={{ padding:"0 20px" }}>
          ...
          <div style={{ position:"relative", marginBottom:12 }}>
            ...
            <div style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", color:"#98A2B3", fontSize:13, pointerEvents:"none" }}>&#12B76A;</div>
          </div>
          ...
          <div style={{ background:"#FFF", border:"1px solid rgba(0,0,0,0.05)", borderRadius:16, overflow:"hidden", marginBottom:16 }}>
            {exosList.length === 0 && (
              <div style={{ padding:"20px", textAlign:"center", fontSize:13, color:"#98A2B3", fontFamily:DISP_F }}>
...
export function CreateSeanceModal({ prog, setProg, setCalSess, push, onClose, C }) {
  ...
  const nextBtn = { flex:2,padding:"16px",borderRadius:16,border:"none",fontFamily:DISP_F,fontSize:14,fontWeight:700,
    cursor:nextOk?"pointer":"default",background:nextOk?"linear-gradient(180deg,#3C5BFF,#2E48D9)":C.s2,
    color:nextOk?"#FFF":"${C.dim}",boxShadow:nextOk?"0 8px 24px rgba(60,91,255,0.25)":"none" };
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 20px 0",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
          ...
          {!newExForm && step===1 && (<>
            ...
            <div style={{display:"flex",gap:8,marginBottom:20}}>
              {DURS.map(d=>{const on=duree===d;return(
                <button key={d} onClick={()=>setDuree(d)} style={{flex:1,padding:"12px 0",borderRadius:12,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(60,91,255,0.12)":C.s1,color:on?DARK.accent:C.mid,fontFamily:DISP_F,fontSize:11,fontWeight:600,cursor:"pointer",textAlign:"center"}}>{d}</button>
);})}
            </div>
          </>)}
            ...
            <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:16,padding:"16px",marginBottom:12}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                {METHODS.map(mm=>{const on=newExForm.methode===mm;return(
                  <button key={mm} onClick={()=>setNewExForm(f=>({...f,methode:mm}))} style={{padding:"4px 12px",borderRadius:999,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(60,91,255,0.12)":C.s1,color:on?DARK.accent:C.mid,cursor:"pointer",fontSize:11,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
);})}
              </div>
            </div>
          </>)}
          ...
          {!newExForm && step===2 && (<>
            ...
            {exos.map((ex,i)=>{
              ...
              return (
                <div key={i} style={{background:C.s1,border:`1px solid ${isOpen?colour+"40":C.bd}`,borderRadius:16,marginBottom:8,overflow:"hidden"}}>
                  ...
                  {/* Accordéon édition */}
                  {isOpen && (
                    <div style={{borderTop:`1px solid ${C.bd}`,padding:"12px 16px",background:"rgba(60,91,255,0.05)"}}>
                        ...
                        {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
                          <div key={pp.k}>
                            ...
                            <div style={{display:"flex",gap:4,alignItems:"center"}}>
                              ...
                              <button onClick={()=>{const cur=parseFloat(ex[pp.k])||0;updateField(i,pp.k,String(cur+1));}} style={{width:28,height:28,borderRadius:8,background:"rgba(60,91,255,0.18)",border:"none",cursor:"pointer",fontSize:14,color:DARK.accent}}>+</button>
                            </div>
                          </div>
))}
                      </div>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {METHODS.map(mm=>{const on=ex.methode===mm;return(
                          <button key={mm} onClick={()=>updateField(i,"methode",mm)} style={{padding:"4px 12px",borderRadius:999,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(60,91,255,0.12)":C.s1,color:on?DARK.accent:C.mid,cursor:"pointer",fontSize:10,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
);})}
                      </div>
                    </div>
)}
            ...
            {!search && (
              <div style={{display:"flex",gap:8,overflowX:"auto",padding:"0 0 12px",flexShrink:0,scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
                {Object.keys(EX).map(g=>{const on=groupe===g;return(
                  <button key={g} onClick={()=>setGroupe(g===groupe?null:g)} style={{flexShrink:0,padding:"8px 16px",borderRadius:12,border:`1.5px solid ${on?C.accent:C.bd}`,background:on?"rgba(60,91,255,0.12)":C.s1,color:on?C.accent:C.mid,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:DISP_F,whiteSpace:"nowrap"}}>{g}</button>
);})}
              </div>
)}
            ...
            {searchList.length>0 && searchList.map((ex,i)=>{
              ...
              const col  = cc(ex.cat);
              return (
                <div key={i} style={{background:C.s1,border:`1px solid ${done?"rgba(18,183,106,0.25)":C.bd}`,borderRadius:16,padding:"16px",marginBottom:12,borderLeft:`3px solid ${col}`,boxShadow:`0 8px 24px -16px ${col}`}}>
                  {/* Badge catégorie */}
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>!done&&openPicker(ex)} style={{flex:1,padding:"12px",borderRadius:12,border:"none",fontFamily:DISP_F,fontSize:13,fontWeight:700,cursor:done?"default":"pointer",
                      background:done?"rgba(18,183,106,0.12)":"linear-gradient(180deg,#3C5BFF,#2E48D9)",
                      color:done?"#12B76A":"#FFF",boxShadow:done?"none":"0 6px 18px rgba(60,91,255,0.25)"}}>
                      {done ?" Ajouté" :"+ Ajouter"}
                    </button>
...
export function EditRecordModal({ exData, prog, setProg, push, onClose }) {
  ...
  const saveEdit = () => {
    ...
    if (editIdx ==="new") {
      ...
      push("","Record modifié",`${exData.nom} · ${editKg}kg × ${editReps} reps`);
    }
  ...
  const deleteEntry = (i) => {
    ...
    persistEntries(next);
    push("","Entrée supprimée","Retirée de l'historique.");
    if (editIdx===i) cancelEdit();
  };

  const tileStyle = (field) => ({
    background: C.s1,
    border:`1px solid ${focusField===field ?"rgba(60,91,255,0.5)" : C.bd}`,
    borderRadius: 20, padding:"16px 16px 12px",
    boxShadow: focusField===field ?"0 0 0 3px rgba(60,91,255,0.12)" :"none",
  });
          ...
          {editIdx !== null ? (
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                <div style={tileStyle("kg")}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:12}}>
                    ...
                    <button onClick={()=>setEditKg(k=>String((parseFloat(k)||0)+2.5))} style={btnStepper("rgba(60,91,255,0.18)",DARK.accent)}>+</button>
                  </div>
                </div>
                <div style={tileStyle("reps")}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:12}}>
                    ...
                    <button onClick={()=>setEditReps(r=>String((parseInt(r)||0)+1))} style={btnStepper("rgba(60,91,255,0.18)",DARK.accent)}>+</button>
                  </div>
                </div>
              </div>
              ...
              <div style={{display:"flex",gap:8,marginBottom:16}}>
                {[1,3,5,8,10,12].map(r=>{const on=editReps===String(r);return(
                  <button key={r} onClick={()=>setEditReps(String(r))} style={{flex:1,padding:"8px 0",borderRadius:12,background:on?"rgba(60,91,255,0.12)":C.s1,border:`1px solid ${on?C.accent:C.bd}`,color:on?DARK.accent:"${C.mid}",fontFamily:DISP_F,fontSize:13,fontWeight:600,cursor:"pointer"}}>{r}</button>
);})}
              </div>

              {/* 1RM estimé */}
              <div style={{borderRadius:16,padding:"16px 16px",marginBottom:16,background:"linear-gradient(135deg,rgba(60,91,255,0.12),rgba(46,72,217,0.05))",border:"1px solid rgba(60,91,255,0.25)",display:"flex",alignItems:"center",justifyContent:"space-between",opacity:rm1Edit>0?1:0.45,transition:"opacity .25s"}}>
                <div>
            ...
            <div>
              ...
              {entries.map((h,i)=>{
                ...
                return (
                  <div key={i} style={{background:C.s1,border:`1px solid ${isRecord?"rgba(157,176,255,0.25)":C.bd}`,borderRadius:16,padding:"12px 16px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
                    <div style={{flex:1,cursor:"pointer",minWidth:0}} onClick={()=>openEdit(i)}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                        {isRecord&&<div style={{fontSize:10,fontWeight:700,color:"#3C5BFF",background:"rgba(60,91,255,0.12)",border:"1px solid rgba(60,91,255,0.25)",borderRadius:8,padding:"2px 8px",fontFamily:DISP_F}}>RECORD</div>}
                    ...
                    <button onClick={()=>deleteEntry(i)} style={{background:"rgba(229,72,77,0.08)",border:"1px solid rgba(229,72,77,0.18)",borderRadius:12,padding:"8px 12px",color:"#E5484D",cursor:"pointer",fontSize:11,fontFamily:DISP_F,fontWeight:600,flexShrink:0}}>Sup.</button>
                  </div>
);
              })}

              <button onClick={openNew} style={{width:"100%",marginTop:4,padding:"16px",borderRadius:16,border:"none",fontFamily:DISP_F,fontSize:14,fontWeight:700,cursor:"pointer",background:"linear-gradient(180deg,#3C5BFF,#2E48D9)",color:"#FFF",boxShadow:"0 8px 24px rgba(60,91,255,0.25)"}}>
                + Nouvelle entrée
              </button>
            </div>
)}
  ...
  return (
    <div style={{background:C.s1,border:"1px solid rgba(0,0,0,0.05)",borderRadius:12,marginBottom:8,overflow:"hidden"}}>
      ...
      <div onClick={() => setExpanded(e => !e)} style={{padding:"12px 16px",cursor:"pointer",borderLeft:`3px solid ${cc}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{flex:1}}>
          <div style={{fontSize:13,fontWeight:600,color:"#F6F7F9",marginBottom:4}}>{exData.nom}</div>
          <div style={{fontSize:10,color:"rgba(245,241,232,0.5)"}}>
            Record : <span style={{fontWeight:600,color:"#F6F7F9"}}>{exData.bestKg}kg × {exData.bestReps} reps</span>
            {"  ·"}
      ...
      {/* Détail déplié */}
      {expanded && (
        <div style={{borderTop:"1px solid rgba(0,0,0,0.05)",padding:"12px 16px",background:"rgba(60,91,255,0.05)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,marginBottom:12}}>
            {Object.entries(OBJ_TARGET).map(([id, obj]) => {
              ...
              return (
                <div key={id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:isCurrentObj?`${obj.color}10`:"#1A1F27",border:`1px solid ${isCurrentObj?obj.color:"rgba(0,0,0,0.05)"}`,borderRadius:8}}>
                  <div>
              ...
              {exData.historique.slice(-3).reverse().map((h, i) => {
                ...
                return (
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    ...
                    <div style={{fontSize:11,fontWeight:500,color:"#F6F7F9"}}>{h.poids}kg × {h.reps} reps</div>
                    <div style={{fontSize:10,color:isRecord?"#3C5BFF":cc,fontWeight:isRecord?700:400}}>{isRecord?"":""}{rm}kg 1RM</div>
                  </div>
);
              })}
            </div>
