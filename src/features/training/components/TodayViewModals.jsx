    ...
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(15,25,35,0.5)", display:"flex", alignItems:"flex-end", justifyContent:"center" }}
      ...
      <div style={{ width:"100%", maxWidth:480, background:"#FFF", borderRadius:"20px 20px 0 0", maxHeight:"92vh", overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 20px 16px" }}>
          <div>
            <div style={{ fontSize:10, color:C.mid, fontWeight:700, letterSpacing:"1.4px", textTransform:"uppercase", marginBottom:4, fontFamily:DISP_F }}>Nouveau record</div>
            <div style={{ fontFamily:SERIF_F, fontSize:20, fontWeight:700, color:C.text, letterSpacing:-0.5 }}>
              Ajouter un <span style={{ fontStyle:"italic", color:"#60A5FA" }}>exercice</span>
            </div>
          </div>
          <button onClick={onClose} style={{ background:C.s1, border:"1px solid rgba(0,0,0,0.06)", borderRadius:12, width:38, height:38, color:C.mid, cursor:"pointer", fontSize:20, display:"grid", placeItems:"center" }}>x</button>
        </div>
        ...
        <div style={{ padding:"0 20px" }}>
          ...
          <div style={{ position:"relative", marginBottom:12 }}>
            <input value={search} onChange={e => { setSearch(e.target.value); setGroupe(null); }}
              ...
              style={{ width:"100%", padding:"12px 16px 12px 32px", background:C.bg, border:"1px solid rgba(0,0,0,0.06)", borderRadius:16, fontSize:14, color:C.text, fontFamily:DISP_F, boxSizing:"border-box", outline:"none" }}/>
          ...
          <div style={{ background:"#FFF", border:"1px solid rgba(0,0,0,0.06)", borderRadius:16, overflow:"hidden", marginBottom:16 }}>
            {exosList.length === 0 && (
            ...
            {exosList.map((ex, i) => {
              ...
              return (
                <div key={i} onClick={() => handleSelect(ex)}
                  style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 16px", borderBottom: notLast ? "1px solid rgba(0,0,0,0.04)" : "none", cursor:"pointer" }}>
                  ...
                  {bestRM && (
                    <div style={{ fontSize:13, fontWeight:700, color:"#60A5FA", fontFamily:DISP_F, flexShrink:0 }}>
                      {bestRM} kg
                    </div>
                  )}
      ...
      <div style={{...SHEET_BASE, maxHeight:"90vh", display:"flex", flexDirection:"column"}}>
        ...
        {!newExForm && (
          <div style={{display:"flex",gap:8,padding:"12px 20px 0",flexShrink:0}}>
            {[1,2].map(i=><div key={i} style={{flex:1,height:4,borderRadius:2,background:step>=i?C.accent:"rgba(0,0,0,0.06)",transition:"background .3s"}}/>)}
          </div>
        )}
          ...
          {!newExForm && step===1 && (<>
            ...
            <div style={{display:"flex",gap:8,marginBottom:20}}>
              {DURS.map(d=>{const on=duree===d;return(
                <button key={d} onClick={()=>setDuree(d)} style={{flex:1,padding:"12px 0",borderRadius:12,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#60A5FA":C.mid,fontFamily:DISP_F,fontSize:11,fontWeight:600,cursor:"pointer",textAlign:"center"}}>{d}</button>
              );})}
            </div>
          </>)}
            ...
            <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:16,padding:"16px",marginBottom:12}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                {METHODS.map(mm=>{const on=newExForm.methode===mm;return(
                  <button key={mm} onClick={()=>setNewExForm(f=>({...f,methode:mm}))} style={{padding:"4px 12px",borderRadius:999,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#60A5FA":C.mid,cursor:"pointer",fontSize:11,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
                );})}
              </div>
            </div>
          </>)}
              ...
              return (
                <div key={i} style={{background:C.s1,border:`1px solid ${isOpen?colour+"40":C.bd}`,borderRadius:16,marginBottom:8,overflow:"hidden"}}>
                  <div style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
                    {/* Numéro carré coloré */}
                    <div style={{width:38,height:38,borderRadius:12,flexShrink:0,background:`linear-gradient(145deg,${colour}30,${colour}08)`,border:`1px solid ${colour}40`,color:colour,display:"grid",placeItems:"center",fontFamily:DISP_F,fontSize:13,fontWeight:700}}>{i+1}</div>
                    <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={()=>setEditEx(m=>({...m,[i]:!m[i]}))}>
                      ...
                      <div style={{fontSize:11,color:"${C.dim}",marginTop:2,fontFamily:DISP_F}}>
                        ...
                        <span style={{color:"#60A5FA",marginLeft:8}}>{isOpen?"▲":"✏️"}</span>
                      </div>
                    </div>
                    <button onClick={()=>removeEx(i)} style={{background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.20)",borderRadius:8,padding:"8px 12px",color:"#F87171",cursor:"pointer",fontSize:11,fontFamily:DISP_F,fontWeight:600}}>×</button>
                  </div>
                  {/* Accordéon édition */}
                  {isOpen && (
                    <div style={{borderTop:`1px solid ${C.bd}`,padding:"12px 16px",background:"rgba(59,130,246,0.03)"}}>
                        ...
                        {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
                          <div key={pp.k}>
                            ...
                            <div style={{display:"flex",gap:4,alignItems:"center"}}>
                              ...
                              <button onClick={()=>{const cur=parseFloat(ex[pp.k])||0;updateField(i,pp.k,String(cur+1));}} style={{width:28,height:28,borderRadius:8,background:"rgba(59,130,246,0.16)",border:"none",cursor:"pointer",fontSize:14,color:"#60A5FA"}}>+</button>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                        {METHODS.map(mm=>{const on=ex.methode===mm;return(
                          <button key={mm} onClick={()=>updateField(i,"methode",mm)} style={{padding:"4px 12px",borderRadius:999,border:`1px solid ${on?C.accent:C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#60A5FA":C.mid,cursor:"pointer",fontSize:10,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
                        );})}
                      </div>
                    </div>
                  )}
            ...
            {searchList.length>0 && searchList.map((ex,i)=>{
              ...
              return (
                <div key={i} style={{background:C.s1,border:`1px solid ${done?"rgba(52,211,153,0.25)":C.bd}`,borderRadius:16,padding:"16px",marginBottom:12,borderLeft:`3px solid ${col}`,boxShadow:`0 8px 24px -16px ${col}`}}>
                  {/* Badge catégorie */}
                  <span style={{fontSize:11,fontWeight:700,letterSpacing:"1.2px",padding:"4px 8px",borderRadius:8,display:"inline-block",marginBottom:8,color:col,background:`${col}18`,border:`1px solid ${col}35`}}>
                    {ex.cat?.toUpperCase()}
                  </span>
                  {/* Nom + info */}
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button onClick={()=>setGuideEx(ex.raw||ex)} style={{padding:"12px 16px",background:"rgba(59,130,246,0.10)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:12,color:"#60A5FA",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:DISP_F}}>
                      Guide →
                    </button>
                  </div>
                </div>
    ...
    <div style={SHEET_OVERLAY} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{...SHEET_BASE, maxHeight:"88vh", overflowY:"auto", WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"8px 20px 16px"}}>
          <div>
            <div style={{fontSize:10,fontWeight:700,letterSpacing:"1.4px",textTransform:"uppercase",color:"${C.dim}",marginBottom:4,fontFamily:DISP_F}}>Modifier le record</div>
            <div style={{fontFamily:SERIF_F,fontSize:20,letterSpacing:-0.5,lineHeight:1}}>{exData.nom}</div>
            <div style={{fontSize:11,color:C.mid,marginTop:4,fontFamily:DISP_F}}>1RM actuel : <span style={{color:"#60A5FA",fontWeight:700}}>{exData.rm1} kg</span></div>
          </div>
        ...
        <div style={{padding:"4px 20px 24px"}}>
          ...
          {editIdx !== null ? (
            <div>
              <div style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",margin:"4px 0 12px",fontFamily:DISP_F}}>
                {editIdx==="new" ? "Nouvelle entrée" : `Modifier l'entrée ${editIdx+1}`}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                <div style={tileStyle("kg")}>
                  <div style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",marginBottom:8,fontFamily:DISP_F}}>Charge</div>
                  <div style={{display:"flex",alignItems:"baseline",justifyContent:"center",gap:4,height:46}}>
                    <input type="number" inputMode="decimal" value={editKg} onChange={e=>setEditKg(e.target.value)}
                      ...
                      style={{width:"100%",background:"none",border:"none",color:"${C.text}",fontFamily:DISP_F,fontSize:44,fontWeight:700,letterSpacing:-2,textAlign:"center",outline:"none",padding:0,minWidth:0}}/>
                ...
                <div style={tileStyle("reps")}>
                  <div style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",marginBottom:8,fontFamily:DISP_F}}>Répétitions</div>
                  <div style={{display:"flex",alignItems:"baseline",justifyContent:"center",gap:4,height:46}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:16}}>
                {[1,3,5,8,10,12].map(r=>{const on=editReps===String(r);return(
                  <button key={r} onClick={()=>setEditReps(String(r))} style={{flex:1,padding:"8px 0",borderRadius:12,background:on?"rgba(59,130,246,0.14)":C.s1,border:`1px solid ${on?C.accent:C.bd}`,color:on?"#60A5FA":"${C.mid}",fontFamily:DISP_F,fontSize:13,fontWeight:600,cursor:"pointer"}}>{r}</button>
                );})}
              </div>

              {/* 1RM estimé */}
              <div style={{borderRadius:16,padding:"16px 16px",marginBottom:16,background:"linear-gradient(135deg,rgba(59,130,246,0.14),rgba(37,99,235,0.05))",border:"1px solid rgba(59,130,246,0.22)",display:"flex",alignItems:"center",justifyContent:"space-between",opacity:rm1Edit>0?1:0.45,transition:"opacity .25s"}}>
                <div>
                  <div style={{fontSize:10,fontWeight:700,letterSpacing:"1.2px",textTransform:"uppercase",color:"#60A5FA",marginBottom:4,fontFamily:DISP_F}}>1RM estimé</div>
                ...
                <div style={{fontFamily:SERIF_F,fontSize:34,color:"#FFF",lineHeight:1}}>
                  {rm1Edit>0?rm1Edit:"—"}{rm1Edit>0&&<span style={{fontSize:13,color:"#60A5FA",fontFamily:DISP_F,fontWeight:700,marginLeft:4}}>kg</span>}
                </div>
              </div>
            ...
            /* Liste des entrées */
            <div>
              <div style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:"${C.dim}",margin:"4px 0 12px",fontFamily:DISP_F}}>Historique ({entries.length})</div>
              ...
              {entries.map((h,i)=>{
                ...
                return (
                  <div key={i} style={{background:C.s1,border:`1px solid ${isRecord?"rgba(96,165,250,0.3)":C.bd}`,borderRadius:16,padding:"12px 16px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
                    <div style={{flex:1,cursor:"pointer",minWidth:0}} onClick={()=>openEdit(i)}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                        {isRecord&&<div style={{fontSize:10,fontWeight:700,color:"#FFAB5D",background:"rgba(255,171,93,0.12)",border:"1px solid rgba(255,171,93,0.3)",borderRadius:8,padding:"2px 8px",fontFamily:DISP_F}}>RECORD</div>}
                        <div style={{fontSize:14,fontWeight:700,color:"${C.text}",fontFamily:DISP_F}}>{h.poids} kg × {h.reps} rep{h.reps>1?"s":""}</div>
                      </div>
                      <div style={{fontSize:10,color:C.mid,fontFamily:DISP_F}}>{h.date} · 1RM : <span style={{color:"#60A5FA",fontWeight:600}}>{rm} kg</span> · <span style={{color:"#60A5FA"}}>Appuie pour modifier</span></div>
                    </div>
                    <button onClick={()=>deleteEntry(i)} style={{background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.2)",borderRadius:12,padding:"8px 12px",color:"#F87171",cursor:"pointer",fontSize:11,fontFamily:DISP_F,fontWeight:600,flexShrink:0}}>Sup.</button>
                  </div>
                );
              })}

              <button onClick={openNew} style={{width:"100%",marginTop:4,padding:"16px",borderRadius:16,border:"none",fontFamily:DISP_F,fontSize:14,fontWeight:700,cursor:"pointer",background:"linear-gradient(180deg,#3B82F6,#2563EB)",color:"#FFF",boxShadow:"0 8px 24px rgba(59,130,246,0.30)"}}>
                + Nouvelle entrée
              </button>
            </div>
          )}
    ...
    <div style={{background:C.s1,border:"1px solid rgba(0,0,0,0.06)",borderRadius:12,marginBottom:8,overflow:"hidden"}}>
      ...
      <div onClick={() => setExpanded(e => !e)} style={{padding:"12px 16px",cursor:"pointer",borderLeft:`3px solid ${cc}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{textAlign:"center",padding:"8px 12px",background:`${target.color}0d`,border:`1px solid ${target.color}35`,borderRadius:8,flexShrink:0,marginLeft:12}}>
          <div style={{fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",fontSize:20,fontWeight:400,color:target.color,lineHeight:1}}>{kgCible > 0 ? kgCible : "—"}<span style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontFamily:"'Inter',sans-serif",fontWeight:400}}>{kgCible>0?" kg":""}</span></div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginLeft:8,flexShrink:0}}>
          {onEdit && <button onClick={e=>{e.stopPropagation();onEdit(exData);}} style={{padding:"4px 8px",background:"rgba(59,130,246,0.10)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:8,color:"#4D8BFF",cursor:"pointer",fontSize:10,fontWeight:600}}>✏️</button>}
      ...
      {/* Détail déplié */}
      {expanded && (
        <div style={{borderTop:"1px solid rgba(0,0,0,0.06)",padding:"12px 16px",background:"rgba(59,130,246,0.02)"}}>
          <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:8}}>Tableau de charges complet</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,marginBottom:12}}>
            {Object.entries(OBJ_TARGET).map(([id, obj]) => {
              const kg = calcKgFor(exData.rm1, obj.reps);
              const isCurrentObj = id === objectif;
              return (
                <div key={id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:isCurrentObj?`${obj.color}10`:"#1C2440",border:`1px solid ${isCurrentObj?obj.color:"rgba(0,0,0,0.06)"}`,borderRadius:8}}>
                  <div>
          ...
          {exData.historique && exData.historique.length > 0 && (
            <div style={{paddingTop:12,borderTop:"1px solid rgba(0,0,0,0.06)"}}>
              <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:8}}>Historique récent</div>
