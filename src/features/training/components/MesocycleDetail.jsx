...
export default function MesocycleDetail({ prog, semC, baseVol, MEV, MAV, MRV, curVol, currentWeek, WEEKS, cycleStart, checkedEx, onClose, mode = "analyse" }) {
  ...
  const card = (key, children) => (
    <div onClick={()=>setExp(exp===key?null:key)} style={{background:C.s1,border:`1px solid ${exp===key?"rgba(59,130,246,0.3)":C.bd}`,borderRadius:20,padding:16,marginBottom:12,cursor:"pointer"}}>
      {children}
    </div>
  );
  const expandRow = (key, label) => (
    <div style={{fontSize:10,color:"#60A5FA",marginTop:12,display:"flex",alignItems:"center",gap:4,fontFamily:DISP_F}}>
      {exp===key?"▴":"▾"} {label}
    </div>
  );
  ...
  const reco = (icon, txt) => (
    <div style={{marginTop:12,padding:"12px 12px",background:"rgba(59,130,246,0.07)",border:"1px solid rgba(59,130,246,0.18)",borderRadius:12,display:"flex",gap:8,alignItems:"flex-start"}}>
      <span style={{fontSize:14,flexShrink:0}}>{icon}</span><div style={{fontSize:11,color:C.mid,lineHeight:1.5}}>{txt}</div>
    </div>
  );
  const lbl = {fontSize:10,fontWeight:700,letterSpacing:"1.2px",textTransform:"uppercase",color:C.dim,marginBottom:8,fontFamily:DISP_F};
    ...
    <div style={{position:"fixed",inset:0,zIndex:340,background:C.bg,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      <div style={{padding:"0 16px 32px",maxWidth:480,margin:"0 auto"}}>
        ...
        <div style={{position:"sticky",top:0,background:C.bg,paddingTop:20,paddingBottom:12,zIndex:2}}>
          <button onClick={onClose} style={{background:"transparent",border:"none",color:"#60A5FA",cursor:"pointer",fontSize:14,fontWeight:600,fontFamily:DISP_F,display:"flex",alignItems:"center",gap:4,marginBottom:16}}>
          ...
          <div style={{fontSize:10,fontWeight:700,letterSpacing:"1.8px",textTransform:"uppercase",color:C.blue,fontFamily:DISP_F,marginBottom:4}}>{isForce ? "Suivi de force · 1RM estimé" : `Mésocycle · Semaine ${currentWeek+1} / 6`}</div>
          <div style={{fontFamily:SERIF_F,fontSize:26,letterSpacing:-0.8,lineHeight:1.1}}>{isForce ? <>Progression <span style={{fontStyle:"italic",color:"#60A5FA"}}>de force</span></> : <>Analyse <span style={{fontStyle:"italic",color:"#60A5FA"}}>de charge</span></>}</div>
            ...
            {hasRatio && (
              <div style={{display:"flex",gap:12,marginTop:16}}>
                {[{l:"Charge 7j",v:acute+" kg"},{l:"Moyenne/sem 28j",v:chronic+" kg"}].map(({l,v})=>(
                  <div key={l} style={{flex:1,background:"rgba(0,0,0,0.03)",borderRadius:12,padding:"8px 12px"}}>
          ...
          return <>
            ...
            <div style={{marginTop:16}}>
              {signals.map(({ic,t,s,st,col,bg},k)=>(
                <div key={k} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:k<3?"1px solid rgba(0,0,0,0.03)":"none"}}>
            ...
            {(() => {
              ...
              if (!any) return (
                <div style={{marginTop:8,padding:"12px",background:"rgba(99,102,241,0.05)",border:"1px dashed rgba(99,102,241,0.22)",borderRadius:12,fontSize:11,color:C.mid,textAlign:"center",fontFamily:DISP_F}}>
                  😴 Note ton sommeil depuis l'Accueil pour voir ta courbe ici.
                </div>
              );
              const maxH = Math.max(...series.map(s=>s.h), slpTgt, 9);
              return (
                <div style={{marginTop:12,padding:"12px 12px 8px",background:"rgba(0,0,0,0.02)",borderRadius:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span style={{fontSize:10,fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",color:C.dim,fontFamily:DISP_F}}>Sommeil · 14 jours</span>
                    <span style={{fontSize:10,color:"#9CA3AF",fontFamily:DISP_F}}>cible {slpTgt}h</span>
                  </div>
                  <div style={{position:"relative",display:"flex",alignItems:"flex-end",gap:4,height:54}}>
                    {/* ligne cible */}
        ...
        {isForce && card("rm", (() => {
          ...
          return <>
                      ...
                      return (
                        <button key={nm} onClick={()=>{setSelEx(nm);setExMenu(false);}}
                          ...
                          {!logged && <span style={{fontSize:10,fontWeight:700,color:C.dim,
                            background:"rgba(0,0,0,0.04)",padding:"4px 8px",borderRadius:8,flexShrink:0,
            ...
            {hasProg ? (
              ...
              <div style={{marginTop:16,padding:"16px",background:"rgba(59,130,246,0.05)",border:"1px dashed rgba(59,130,246,0.2)",borderRadius:12,fontSize:13,color:C.mid,textAlign:"center",fontFamily:DISP_F}}>
