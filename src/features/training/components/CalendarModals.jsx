    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:32}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 16px 16px"}}>
          <div>
            <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",marginBottom:4}}>Cardio</div>
          ...
          <button onClick={onClose} style={{background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,width:36,height:36,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:20}}>×</button>
        </div>
        ...
        <div style={{padding:"0 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                {ERGOS.map(e => (
                  <div key={e.id} onClick={() => { setErgo(e); const defs={}; e.params.forEach(p=>{ defs[p.k]=p.def||""; }); setParams(defs); setStep(1); }}
                    style={{padding:"16px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",gap:12,transition:"border-color .15s"}}
          ...
          {step === 1 && ergo && (
            <div>
              ...
              {/* Durée */}
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Durée</div>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <button onClick={() => setDuree(d => Math.max(5,d-5))} style={{width:36,height:36,borderRadius:8,background:C.s2,border:"none",cursor:"pointer",fontSize:20,fontWeight:400,color:"rgba(245,241,232,0.50)"}}>−</button>
                ...
                <div style={{display:"flex",gap:8,marginTop:12}}>
                  {[15,20,30,45,60,90].map(d => (
                    <button key={d} onClick={() => setDuree(d)} style={{flex:1,padding:"4px 2px",background:duree===d?"rgba(59,130,246,0.1)":"transparent",border:`0.5px solid ${duree===d?"#4D8BFF":"rgba(190,180,255,0.07)"}`,borderRadius:8,color:duree===d?"#4D8BFF":"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:10,fontWeight:duree===d?600:400}}>{d}'</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Paramètres</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {ergo.params.map(p => (
                    <div key={p.k}>
                      <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginBottom:4,fontWeight:500}}>{p.l}{p.unit&&p.unit!=="ex: 2:10"&&p.unit!=="ex: 2:20"&&p.unit!=="ex: 2:05"&&p.unit!=="ex: 2:15" ? ` (${p.unit})` : ""}</div>
                      {p.opts ? (
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:4}}>
                          <input
                            ...
                            style={{flex:1,padding:"8px 12px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:13,color:"#F5F1E8",fontFamily:"'Inter',sans-serif"}}
                          />
              ...
              {/* Zones cardiaques */}
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Zone cardiaque</div>
                <div style={{display:"flex",gap:4,marginBottom:12}}>
                  {ZONES.map(z => (
                    <button key={z.id} onClick={() => setZone(z.id)} style={{flex:1,padding:"8px 2px",textAlign:"center",background:zone===z.id?`${z.color}15`:"transparent",border:`1px solid ${zone===z.id?z.color:"rgba(190,180,255,0.07)"}`,borderRadius:8,cursor:"pointer",transition:"all .12s"}}>
                ...
                <div style={{marginTop:12,display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <input value={bpm} onChange={e=>setBpm(e.target.value)} placeholder="ex: 145 (facultatif)" style={{flex:1,padding:"8px 12px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:13,color:"#F5F1E8",fontFamily:"'Inter',sans-serif"}}/>
                </div>
              </div>

              {/* Calories */}
              <div style={{background:C.s1,border:`1px solid ${kcalFinal>0?"rgba(59,130,246,0.2)":"rgba(190,180,255,0.07)"}`,borderRadius:12,padding:"16px 16px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase"}}>Calories estimées</div>
                ...
                {editKcal ? (
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <span style={{fontSize:13,color:"rgba(245,241,232,0.50)"}}>kcal</span>
                  </div>
                ) : (
                  <div style={{display:"flex",alignItems:"baseline",gap:8}}>
                    <div style={{fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",fontSize:34,fontWeight:400,color:kcalAuto>0?"#4D8BFF":"rgba(245,241,232,0.32)",lineHeight:1}}>{kcalAuto>0?kcalAuto:"—"}</div>
                    {kcalAuto>0&&<div style={{fontSize:13,color:"rgba(245,241,232,0.50)"}}>kcal</div>}
                  </div>
                )}
                {kcalAuto>0&&!editKcal&&(
              ...
              <button onClick={onClose} style={{width:"100%",padding:"12px",background:"transparent",border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:13,fontFamily:"'Inter',sans-serif"}}>Annuler</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:32}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 16px 16px"}}>
          <div>
            <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",marginBottom:4}}>Activité sportive</div>
            <div style={{fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",fontSize:20,fontWeight:400,color:"#F5F1E8"}}>
        ...
        <div style={{padding:"0 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {SPORTS.map(s => (
                  <div key={s.id} onClick={() => setSport(s)}
                    style={{padding:"12px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"border-color .15s"}}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginTop:1}}>~{Math.round(s.met*70)} kcal/h</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {sport && (
            <div>
              ...
              {/* Calories */}
              <div style={{background:C.s1,border:`1px solid ${kcalAuto>0?"rgba(59,130,246,0.2)":"rgba(190,180,255,0.07)"}`,borderRadius:12,padding:"16px 16px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase"}}>Calories brûlées <span style={{color:"rgba(245,241,232,0.50)",fontWeight:400,fontSize:8}}>(facultatif)</span></div>
                  <button onClick={() => { setEditKcal(e=>!e); setKcalManuel(String(kcalAuto)); }} style={{fontSize:10,color:"#4D8BFF",background:"transparent",border:"none",cursor:"pointer",fontWeight:600}}>{editKcal?"Auto":"Modifier"}</button>
                </div>
                {editKcal ? (
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <div>
                    <div style={{display:"flex",alignItems:"baseline",gap:8}}>
                      ...
                      <div style={{fontSize:13,color:"rgba(245,241,232,0.50)"}}>kcal</div>
                    </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:32}}>
        <div style={{padding:"20px 16px 0",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1}}>
            <div style={{display:"inline-block",padding:"4px 12px",background:`${cc}14`,border:`0.5px solid ${cc}40`,borderRadius:8,fontSize:10,color:cc,letterSpacing:"1px",textTransform:"uppercase",fontWeight:600,marginBottom:12}}>{exData.cat}</div>
          ...
          <button onClick={onClose} style={{background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,width:36,height:36,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:20,flexShrink:0,marginLeft:12}}>×</button>
        </div>
        <div style={{padding:"12px 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
          {[{l:"Séries",v:exSerie?.series||exData.s},{l:"Reps",v:exSerie?.reps||exData.r},{l:"Repos",v:exSerie?.repos||exData.rest},{l:"Charge",v:exSerie?.charge||exData.ch}].map(s => (
            <div key={s.l} style={{padding:"8px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,textAlign:"center",flex:1,minWidth:60}}>
              <div style={{fontSize:14,fontWeight:400,color:"#4D8BFF",fontFamily:"'Outfit','DM Sans',system-ui,sans-serif"}}>{s.v||"—"}</div>
              <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginTop:2}}>{s.l}</div>
            </div>
          ))}
        </div>
        <Tabs
        ...
        <div style={{padding:"0 16px"}}>
          {tab==="tips" && (
            <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px"}}>
              <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Conseils techniques</div>
              {(exData.tips||[]).map((tip,i) => (
                <div key={i} style={{display:"flex",gap:12,marginBottom:16,paddingBottom:16,borderBottom:i<(exData.tips||[]).length-1?"0.5px solid rgba(190,180,255,0.07)":"none"}}>
              ...
              {exData.prog && <div style={{marginTop:4,padding:"12px 12px",background:"rgba(34,197,94,0.08)",border:"0.5px solid rgba(34,197,94,0.2)",borderRadius:8}}><div style={{fontSize:10,color:"#5FE0A5",fontWeight:500,textTransform:"uppercase",marginBottom:4}}>Progression</div><div style={{fontSize:13,color:"rgba(245,241,232,0.50)",lineHeight:1.5}}>{exData.prog}</div></div>}
            </div>
          )}
          {tab==="variantes" && (
            <div>
              {(exData.variantes||[]).map((v,i) => (
                <div key={i} style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px",marginBottom:8}}>
          ...
          {tab==="erreurs" && (
            <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px"}}>
              <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Erreurs à éviter</div>
          ...
          {tab==="morpho" && (
            <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px"}}>
              <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>Adaptation morphologique</div>
              {(exData.morpho||"").split("\n").filter(Boolean).map((line,i,arr) => (
                <div key={i} style={{display:"flex",gap:8,marginBottom:12,paddingBottom:12,borderBottom:i<arr.length-1?"0.5px solid rgba(190,180,255,0.07)":"none",alignItems:"flex-start"}}>
              ...
              {!(exData.morpho||"").includes("\n") && exData.morpho && <div style={{fontSize:13,color:"#F5F1E8",lineHeight:1.7}}>{exData.morpho}</div>}
            </div>
          )}
        </div>
        <div style={{padding:"16px 16px 0"}}>
          <button onClick={onClose} style={{width:"100%",padding:"12px",background:"transparent",border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:13,fontFamily:"'Inter',sans-serif"}}>← Retour à la séance</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{padding:"20px 16px",paddingBottom:32}}>
        ...
        <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:16}}>{ex.series}×{ex.reps} · Repos {ex.repos}{ex.charge?` · ${ex.charge}`:""}</div>

        {!dbEx && (
          <div style={{padding:"16px 16px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,textAlign:"center"}}>
            <div style={{fontSize:26,marginBottom:8}}>📖</div>
            <div style={{fontSize:13,color:"rgba(245,241,232,0.50)",lineHeight:1.5,marginBottom:8}}>Aucune information détaillée n'est disponible pour cet exercice dans la bibliothèque.</div>
        ...
        {dbEx && (
          <>
            ...
            {dbEx.tips?.length > 0 && (
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                <div style={{fontSize:10,color:"#5FE0A5",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>💡 Tips techniques</div>
                {dbEx.tips.map((tip,i) => (
            ...
            {dbEx.erreurs?.length > 0 && (
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                <div style={{fontSize:10,color:"#FF7A6B",fontWeight:700,letterSpacing:"1px",textTransform:"uppercase",marginBottom:12}}>⚠️ Erreurs à éviter</div>
            ...
            {onOpenGuide && (
              <button onClick={()=>onOpenGuide(dbEx,ex)} style={{width:"100%",padding:"12px",background:"rgba(59,130,246,0.06)",border:"0.5px solid rgba(59,130,246,0.25)",borderRadius:12,color:"#4D8BFF",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",marginTop:4}}>
        ...
        <button onClick={onClose} style={{width:"100%",padding:"12px",marginTop:12,background:"transparent",border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,color:"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:13,fontFamily:"'Inter',sans-serif"}}>Fermer</button>
      </div>
    </div>
  );
}
...
export function ExerciceEditable({ ex, exIdx, jourIdx, prog, setProg, cc, METHODS, onGuide, onInfo, checkedEx, toggleCheck, seanceId }) {
  ...
  return (
    <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:12,marginBottom:8,overflow:"hidden",opacity:isChecked?0.6:1,transition:"opacity .15s"}}>
     <div style={{padding:"12px 12px",borderLeft:`3px solid ${cc}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
       {toggleCheck && (
        <div onClick={(e)=>{e.stopPropagation();toggleCheck(seanceId,exIdx,ex.repos);}} style={{width:22,height:22,borderRadius:8,background:isChecked?"#5FE0A5":"transparent",border:`2px solid ${isChecked?"#5FE0A5":"rgba(190,180,255,0.07)"}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:13,color:DARK.surface,marginTop:1,transition:"all .15s"}}>{isChecked?"✓":""}</div>
       )}
       <div style={{flex:1,cursor:"pointer"}} onClick={() => setEditing(e=>!e)}>
        <div style={{fontSize:13,fontWeight:600,color:isChecked?"rgba(245,241,232,0.50)":"#F5F1E8",marginBottom:4,textDecoration:isChecked?"line-through":"none"}}>{ex.nom}</div>
       ...
       <div style={{display:"flex",gap:4,flexShrink:0}}>
        {onInfo && <button onClick={(e)=>{e.stopPropagation();onInfo(ex);}} title="Infos" style={{padding:"4px 8px",background:"rgba(6,182,212,0.06)",border:"0.5px solid rgba(6,182,212,0.25)",borderRadius:8,color:"#06B6D4",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:"'Inter',sans-serif"}}>i</button>}
        {dbEx && onGuide && <button onClick={(e)=>{e.stopPropagation();onGuide(dbEx,ex);}} style={{padding:"4px 8px",background:"rgba(59,130,246,0.06)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:8,color:"#4D8BFF",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"'Inter',sans-serif"}}>Guide ›</button>}
        <button onClick={(e)=>{e.stopPropagation();setEditing(ed=>!ed);}} style={{padding:"4px 8px",background:"rgba(59,130,246,0.08)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:8,color:"#4D8BFF",cursor:"pointer",fontSize:10}}>✏️</button>
       </div>
      </div>
      {editing && (
       <div style={{marginTop:12,paddingTop:12,borderTop:"0.5px solid rgba(190,180,255,0.07)"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
         {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
          <div key={pp.k}>
           <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginBottom:4,fontWeight:600}}>{pp.l}</div>
           <div style={{display:"flex",gap:4,alignItems:"center"}}>
            ...
            <input value={ex[pp.k]||""} onChange={e=>updateEx(pp.k,e.target.value)} style={{flex:1,padding:"4px 4px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:11,textAlign:"center",fontFamily:"'Inter',sans-serif"}}/>
        ...
        <div style={{marginBottom:8}}>
         <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginBottom:4,fontWeight:600}}>TEMPO</div>
         <input value={ex.tempo||""} onChange={e=>updateEx("tempo",e.target.value)} placeholder="Ex: 2-1-3" style={{width:"100%",padding:"8px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:8,fontSize:11,fontFamily:"'Inter',sans-serif",boxSizing:"border-box"}}/>
        </div>
        <div>
         <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginBottom:4,fontWeight:600}}>MÉTHODE</div>
         <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
          {METHODS.map(mm=>(
           <button key={mm} onClick={()=>updateEx("methode",mm)} style={{padding:"4px 8px",borderRadius:12,border:`1px solid ${ex.methode===mm?"#4D8BFF":"rgba(190,180,255,0.07)"}`,background:ex.methode===mm?"rgba(59,130,246,0.1)":"transparent",color:ex.methode===mm?"#4D8BFF":"rgba(245,241,232,0.50)",cursor:"pointer",fontSize:10,fontFamily:"'Inter',sans-serif"}}>{mm}</button>
          ))}
         </div>
        </div>
       </div>
