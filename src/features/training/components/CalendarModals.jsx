...
const ZONES = [
  {id:"z1",l:"Z1",pct:"50-60%",desc:"Récupération active",color:"#12B76A",facteur:0.85},
  {id:"z2",l:"Z2",pct:"60-70%",desc:"Endurance aérobie",color:"#3C5BFF",facteur:1.0},
  {id:"z3",l:"Z3",pct:"70-80%",desc:"Tempo / Cardio",color:"#3C5BFF",facteur:1.15},
  {id:"z4",l:"Z4",pct:"80-90%",desc:"Seuil anaérobie",color:"#3C5BFF",facteur:1.3},
  {id:"z5",l:"Z5",pct:"90-100%",desc:"VO2max / Sprint",color:"#9DB0FF",facteur:1.5},
];
            ...
            <div>
              ...
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                ...
                <div style={{display:"flex",gap:8,marginTop:12}}>
                  {[15,20,30,45,60,90].map(d => (
                    <button key={d} onClick={() => setDuree(d)} style={{flex:1,padding:"4px 2px",background:duree===d?"rgba(60,91,255,0.12)":"transparent",border:`0.5px solid ${duree===d?"#3C5BFF":"rgba(190,180,255,0.08)"}`,borderRadius:8,color:duree===d?"#3C5BFF":"rgba(245,241,232,0.5)",cursor:"pointer",fontSize:10,fontWeight:duree===d?600:400}}>{d}'</button>
))}
                </div>
              </div>
                    ...
                    <div key={p.k}>
                      ...
                      {p.opts ? (
                        <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                          {p.opts.map(o => (
                            <button key={o} onClick={() => setParams(pr=>({...pr,[p.k]:o}))} style={{padding:"4px 12px",borderRadius:8,border:`0.5px solid ${(params[p.k]||p.def)===o?"#3C5BFF":"rgba(190,180,255,0.08)"}`,background:(params[p.k]||p.def)===o?"rgba(60,91,255,0.08)":"transparent",color:(params[p.k]||p.def)===o?"#3C5BFF":"rgba(245,241,232,0.5)",cursor:"pointer",fontSize:11,fontWeight:500}}>{o}</button>
))}
                        </div>
) : (
                        <div style={{display:"flex",alignItems:"center",gap:4}}>
                          <input
                            ...
                            style={{flex:1,padding:"8px 12px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:8,fontSize:13,color:"#F6F7F9",fontFamily:"'Archivo',sans-serif"}}
                          />
              ...
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                ...
                <div style={{marginTop:12,display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <input value={bpm} onChange={e=>setBpm(e.target.value)} placeholder="ex: 145 (facultatif)" style={{flex:1,padding:"8px 12px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:8,fontSize:13,color:"#F6F7F9",fontFamily:"'Archivo',sans-serif"}}/>
                </div>
              </div>

              {/* Calories */}
              <div style={{background:C.s1,border:`1px solid ${kcalFinal>0?"rgba(60,91,255,0.18)":"rgba(190,180,255,0.08)"}`,borderRadius:12,padding:"16px 16px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  ...
                  <button onClick={() => { setEditKcal(e=>!e); setKcalManuel(String(kcalAuto)); }} style={{fontSize:10,color:"#3C5BFF",background:"transparent",border:"none",cursor:"pointer",fontWeight:600}}>{editKcal?"Auto":"Modifier"}</button>
                </div>
                {editKcal ? (
        ...
        <div style={{padding:"0 16px"}}>
          ...
          {sport && (
            <div>
              ...
              {/* Calories */}
              <div style={{background:C.s1,border:`1px solid ${kcalAuto>0?"rgba(60,91,255,0.18)":"rgba(190,180,255,0.08)"}`,borderRadius:12,padding:"16px 16px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,padding:"16px 16px"}}>
              ...
              {(exData.tips||[]).map((tip,i) => (
                <div key={i} style={{display:"flex",gap:12,marginBottom:16,paddingBottom:16,borderBottom:i<(exData.tips||[]).length-1?"0.5px solid rgba(190,180,255,0.08)":"none"}}>
                  <div style={{width:22,height:22,borderRadius:"50%",background:"rgba(60,91,255,0.12)",border:"0.5px solid rgba(60,91,255,0.18)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10,fontWeight:500,color:"#3C5BFF"}}>{i+1}</div>
                  <div style={{fontSize:13,color:"#F6F7F9",lineHeight:1.6}}>{tip}</div>
                </div>
))}
              {exData.prog && <div style={{marginTop:4,padding:"12px 12px",background:"rgba(34,197,94,0.08)",border:"0.5px solid rgba(34,197,94,0.18)",borderRadius:8}}><div style={{fontSize:10,color:"#12B76A",fontWeight:500,textTransform:"uppercase",marginBottom:4}}>Progression</div><div style={{fontSize:13,color:"rgba(245,241,232,0.5)",lineHeight:1.5}}>{exData.prog}</div></div>}
            </div>
)}
          {tab==="variantes" && (
            <div>
              {(exData.variantes||[]).map((v,i) => (
                <div key={i} style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,padding:"16px 16px",marginBottom:8}}>
                  <div style={{fontSize:13,fontWeight:500,color:"#F6F7F9",marginBottom:4}}>{v.nom||v}</div>
            ...
            <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,padding:"16px 16px"}}>
              ...
              {(exData.morpho||"").split("\n").filter(Boolean).map((line,i,arr) => (
                <div key={i} style={{display:"flex",gap:8,marginBottom:12,paddingBottom:12,borderBottom:i<arr.length-1?"0.5px solid rgba(190,180,255,0.08)":"none",alignItems:"flex-start"}}>
                  ...
                  <div style={{fontSize:11,color:"#F6F7F9",lineHeight:1.6,flex:1}}>{line.split(":").slice(1).join(":").trim()}</div>
                </div>
))}
              {!(exData.morpho||"").includes("\n") && exData.morpho && <div style={{fontSize:13,color:"#F6F7F9",lineHeight:1.6}}>{exData.morpho}</div>}
            </div>
)}
        </div>
      ...
      <div style={{padding:"20px 16px",paddingBottom:32}}>
        ...
        {dbEx && (
          <>
            ...
            {dbEx.tips?.length > 0 && (
              <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,padding:"16px 16px",marginBottom:12}}>
                <div style={{fontSize:10,color:"#12B76A",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:12}}> Tips techniques</div>
                {dbEx.tips.map((tip,i) => (
                  <div key={i} style={{display:"flex",gap:12,marginBottom:12,paddingBottom:12,borderBottom:i<dbEx.tips.length-1?"0.5px solid #1A1F27":"none",alignItems:"flex-start"}}>
                    <div style={{width:20,height:20,borderRadius:"50%",background:"rgba(34,197,94,0.12)",border:"0.5px solid rgba(34,197,94,0.25)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10,fontWeight:500,color:"#12B76A",marginTop:1}}>{i+1}</div>
                    <div style={{fontSize:13,color:"#F6F7F9",lineHeight:1.6,flex:1}}>{tip}</div>
                  </div>
))}
              </div>
)}
        ...
        <button onClick={onClose} style={{width:"100%",padding:"12px",marginTop:12,background:"transparent",border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,color:"rgba(245,241,232,0.5)",cursor:"pointer",fontSize:13,fontFamily:"'Archivo',sans-serif"}}>Fermer</button>
      </div>
    </div>
);
}
    ...
    <div style={{background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:12,marginBottom:8,overflow:"hidden",opacity:isChecked?0.6:1,transition:"opacity .15s"}}>
     <div style={{padding:"12px 12px",borderLeft:`3px solid ${cc}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
       {toggleCheck && (
        <div onClick={(e)=>{e.stopPropagation();toggleCheck(seanceId,exIdx,ex.repos);}} style={{width:22,height:22,borderRadius:8,background:isChecked?"#12B76A":"transparent",border:`2px solid ${isChecked?"#12B76A":"rgba(190,180,255,0.08)"}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:13,color:DARK.surface,marginTop:1,transition:"all .15s"}}>{isChecked?"":""}</div>
)}
       <div style={{flex:1,cursor:"pointer"}} onClick={() => setEditing(e=>!e)}>
        <div style={{fontSize:13,fontWeight:600,color:isChecked?"rgba(245,241,232,0.5)":"#F6F7F9",marginBottom:4,textDecoration:isChecked?"line-through":"none"}}>{ex.nom}</div>
      ...
      {editing && (
       <div style={{marginTop:12,paddingTop:12,borderTop:"0.5px solid rgba(190,180,255,0.08)"}}>
         ...
         {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
          <div key={pp.k}>
           ...
           <div style={{display:"flex",gap:4,alignItems:"center"}}>
            ...
            <input value={ex[pp.k]||""} onChange={e=>updateEx(pp.k,e.target.value)} style={{flex:1,padding:"4px 4px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:8,fontSize:11,textAlign:"center",fontFamily:"'Archivo',sans-serif"}}/>
        ...
        <div style={{marginBottom:8}}>
         ...
         <input value={ex.tempo||""} onChange={e=>updateEx("tempo",e.target.value)} placeholder="Ex: 2-1-3" style={{width:"100%",padding:"8px 12px",background:C.s1,border:"0.5px solid rgba(190,180,255,0.08)",borderRadius:8,fontSize:11,fontFamily:"'Archivo',sans-serif",boxSizing:"border-box"}}/>
        </div>
        <div>
         <div style={{fontSize:10,color:"rgba(245,241,232,0.5)",marginBottom:4,fontWeight:600}}>MÉTHODE</div>
         <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
          {METHODS.map(mm=>(
           <button key={mm} onClick={()=>updateEx("methode",mm)} style={{padding:"4px 8px",borderRadius:12,border:`1px solid ${ex.methode===mm?"#3C5BFF":"rgba(190,180,255,0.08)"}`,background:ex.methode===mm?"rgba(60,91,255,0.12)":"transparent",color:ex.methode===mm?"#3C5BFF":"rgba(245,241,232,0.5)",cursor:"pointer",fontSize:10,fontFamily:"'Archivo',sans-serif"}}>{mm}</button>
))}
         </div>
        </div>
       </div>
