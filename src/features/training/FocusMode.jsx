...
// ── RestStage — Chrono de repos entre les séries ─────────────────────────────
function RestStage({ restSec, totalRest, onSkip, nextSet, totalSets }) {
  const pct    = totalRest > 0 ? Math.max(0, restSec / totalRest) : 0;
  const mm     = String(Math.floor(restSec / 60)).padStart(2, '0');
  const ss     = String(restSec % 60).padStart(2, '0');
  const R      = 70;
  const CIRC   = 2 * Math.PI * R;
  const dash   = CIRC * pct;
  const urgent = restSec <= 5 && restSec > 0;
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column',
      ...
      {/* Label */}
      <div style={{ fontFamily:MON, fontSize:9.5, fontWeight:600,
                    color:T.t4, letterSpacing:'2px', marginBottom:28,
      ...
      <div style={{ position:'relative', width:180, height:180,
        ...
        <svg width="180" height="180" viewBox="0 0 180 180"
          ...
          <circle cx="90" cy="90" r={R}
            fill="none"
            stroke={urgent ? '#F87171' : T.ac}
            strokeWidth="8"
            strokeLinecap="round"
            strokeDasharray={`${dash} ${CIRC}`}
            style={{ transition: restSec === totalRest ? 'none' : 'stroke-dasharray .98s linear, stroke .3s' }}/>
        </svg>
        <div style={{ textAlign:'center', zIndex:1 }}>
          <div style={{ fontFamily:MON, fontSize:46, fontWeight:700,
                        color: urgent ? '#F87171' : T.t1,
                        letterSpacing:-1, ...NUM,
                        transition:'color .3s' }}>
            {mm}:{ss}
          </div>
          <div style={{ fontFamily:F, fontSize:11, color:T.t4, marginTop:2, letterSpacing:'1.5px', textTransform:'uppercase' }}>
            Repos
          </div>
        </div>
      </div>

      <div style={{ ...GL, padding:'12px 18px', borderRadius:14, marginBottom:32,
                    maxWidth:290, textAlign:'center' }}>
        <div style={{ fontFamily:F, fontSize:12.5, color:T.t2, lineHeight:1.5 }}>
          💧 Hydrate-toi · respire profondément · prépare la prochaine série
        </div>
      </div>

      <button className="fm-tap" onClick={onSkip} style={{
        padding:'14px 36px', borderRadius:16, border:`1.5px solid ${T.bdAc}`,
        background:T.acSoft, color:T.ac,
        fontFamily:F, fontSize:14, fontWeight:700, cursor:'pointer',
        display:'flex', alignItems:'center', gap:8 }}>
        Passer le repos
        <I n="skip" sz={14} c={T.ac} s={2}/>
      </button>
    </div>
  );
}
...
export default function FocusMode({
  ...
  const [restSec,  setRestSec]  = useState(0);  // countdown repos

  // Parse ex.repos → secondes (ex: "90s", "2min", "90", undefined → 90)
  const parseRepos = (r) => {
    if (!r) return 90;
    const s = String(r).toLowerCase().trim();
    if (s.includes('min')) return Math.round(parseFloat(s) * 60);
    return parseInt(s) || 90;
  };
  ...
  useEffect(() => {
    ...
    if (restSec <= 0) { setPhase('set'); return; }
    const id = setInterval(() => setRestSec(s => {
      if (s <= 1) { clearInterval(id); setPhase('set'); return 0; }
      return s - 1;
    }), 1000);
  ...
  const content = (
    <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0,
      ...
      {phase === 'rest' && (
        <RestStage
          restSec={restSec}
          totalRest={parseRepos(ex?.repos)}
          nextSet={Math.min(setIdx + 1, totalSets)}
          totalSets={totalSets}
          onSkip={() => { setRestSec(0); setPhase('set'); }}
        />
      )}
      {phase === 'done' && (
        <DoneStage
