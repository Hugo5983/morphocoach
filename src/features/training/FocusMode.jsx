    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column',
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column',
        ...
        <div style={{ textAlign:'center', marginBottom:24 }}>
          ...
          {/* Chrono temps de la série (compte en avant) */}
          <div style={{ marginTop:10, display:'inline-flex', alignItems:'center', gap:6 }}>
            <I n="clock" sz={11} c={T.t4} s={2}/>
            <span style={{ fontFamily:MON, fontSize:13, fontWeight:700, color:T.t2,
                           letterSpacing:0.5, ...NUM }}>
              {String(Math.floor(setTimer/60)).padStart(2,'0')}:{String(setTimer%60).padStart(2,'0')}
            </span>
          </div>
        </div>
...
export default function FocusMode({
  ...
  const [setTimer,   setSetTimer]   = useState(0);   // chrono de la série en cours
  ...
  // Reset chrono quand on passe à la prochaine série
  useEffect(() => {
    if (phase === 'set') setSetTimer(0);
  }, [phase, setIdx]);

  useEffect(() => {
    const id = setInterval(() => {
      setElapsed(e => e+1);
      if (phase === 'set') setSetTimer(e => e+1);
    }, 1000);
