...
export default function AnalyseIA(props) {
    ...
    <div style={{padding:"0 15px"}} className="fade-in">
      {loadMsg.startsWith("Erreur") ? (
        <Box style={{textAlign:"center",padding:"40px 20px"}} className="scale-in">
          ...
          <div style={{fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:15,color:C.red,fontWeight:500,marginBottom:10}}>Génération échouée</div>
          <div style={{fontSize:12,color:"rgba(245,241,232,0.50)",marginBottom:20,lineHeight:1.6}}>{loadMsg}</div>
        ...
        <div>
          <Box style={{textAlign:"center",padding:"32px 20px 24px"}} className="slide-up">
            ...
            <div style={{fontFamily:"'Space Grotesk','Inter',system-ui,sans-serif",fontSize:17,color:C.gold,fontWeight:300,marginBottom:6,letterSpacing:"0.5px"}}>{loadMsg}</div>
            <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",lineHeight:1.8}}>Analyse morphologique + génération<br/>du programme personnalisé…</div>
          </Box>
          {[1,2,3].map(i=>(
  ...
  return (
    <div style={{padding:"0 15px"}}>
      ...
      <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginBottom:12,letterSpacing:"0.5px"}}>ÉTAPE {aStep+1}/{steps.length} — {steps[aStep].toUpperCase()}</div>

      {aStep===0 && <Box>
        <Lbl>Photos de posture</Lbl>
        <div style={{padding:"10px 12px",background:"rgba(59,130,246,0.08)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:8,fontSize:12,color:"rgba(245,241,232,0.50)",marginBottom:14,lineHeight:1.6}}>
            ...
            <div onClick={()=>{
              ...
              {photos[key]
                ...
                : <>
                    ...
                    <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginTop:2,textAlign:"center",padding:"0 10px"}}>{desc}</div>
                  </>
              }
              {photos[key]&&(
        ...
        <div style={{marginTop:6,marginBottom:10,fontSize:11,color:"rgba(245,241,232,0.50)",textAlign:"center"}}>
      ...
      {aStep===1 && <Box>
        ...
        <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:4}}>Prénom <span style={{color:C.textMid}}>(facultatif)</span></div>
        <Inp placeholder="Prénom" value={form.prenom} onChange={e=>setForm({...form,prenom:e.target.value})}/>
        <G2>
          <div>
            <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:4}}>Âge <span style={{color:C.red}}>*</span></div>
            <Inp type="number" placeholder="Ex: 28" style={{marginBottom:0}} value={form.age} onChange={e=>setForm({...form,age:e.target.value})}/>
          </div>
          <div>
            <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:4}}>Sexe <span style={{color:C.red}}>*</span></div>
        ...
        <G2 style={{marginTop:6}}>
          <div>
            <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:4}}>Poids (kg) <span style={{color:C.red}}>*</span></div>
          ...
          <div>
            <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:4}}>Taille (cm) <span style={{color:C.red}}>*</span></div>
        ...
        <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:6,marginTop:6}}>Niveau <span style={{color:C.red}}>*</span></div>
        {[{id:"debutant",l:"Débutant",d:"< 1 an"},{id:"intermediaire",l:"Intermédiaire",d:"1-3 ans"},{id:"avance",l:"Avancé",d:"> 3 ans"}].map(n=>(
          <div key={n.id} onClick={()=>setForm({...form,niveau:n.id})} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",background:form.niveau===n.id?C.goldD:C.s2,border:`1px solid ${form.niveau===n.id?C.gold:C.s3}`,borderRadius:9,cursor:"pointer",marginBottom:6}}>
            <span style={{fontSize:13,fontWeight:600}}>{n.l}</span><span style={{fontSize:10,color:"rgba(245,241,232,0.50)"}}>{n.d}</span>
          </div>
        ))}
      ...
      {aStep===2 && <Box>
        <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:8}}>Objectif principal <span style={{color:C.red}}>*</span></div>
        ...
        <textarea style={{width:"100%",padding:"11px 13px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:9,color:C.text,fontSize:13,minHeight:60,resize:"vertical",marginBottom:10,fontFamily:"'Inter',sans-serif"}} placeholder="Décrivez votre objectif précis (facultatif)" value={form.objectifPrecis} onChange={e=>setForm({...form,objectifPrecis:e.target.value})}/>
        {/* Sport pratiqué — facultatif */}
        <div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:6,fontWeight:500}}>Sport pratiqué <span style={{color:"rgba(245,241,232,0.50)",fontWeight:400}}>(facultatif)</span></div>
          <input value={form.sport||""} onChange={e=>setForm({...form,sport:e.target.value})} placeholder="Ex: Football, Tennis, Natation, Boxe…" style={{width:"100%",padding:"10px 13px",background:C.s2,border:"0.5px solid rgba(190,180,255,0.07)",borderRadius:9,color:C.text,fontSize:13,fontFamily:"'Inter',sans-serif",boxSizing:"border-box"}}/>
        </div>
        <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:6}}>Jours d'entraînement <span style={{color:C.red}}>*</span></div>
      ...
      {aStep===3 && <Box>
        ...
        <div style={{padding:"8px 10px",background:"rgba(59,130,246,0.08)",border:"0.5px solid rgba(59,130,246,0.2)",borderRadius:7,fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:10,lineHeight:1.6}}>Exercices correctifs = renforcement uniquement. Consultez un kiné pour tout diagnostic.</div>
      ...
      {aStep===4 && <Box>
        <div style={{fontSize:11,color:"rgba(245,241,232,0.50)",marginBottom:8}}>Matériel disponible <span style={{color:C.red}}>*</span></div>
        ...
        <div onClick={()=>setCorrigerFaibles(v=>!v)} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:corrigerFaibles?"rgba(59,130,246,0.06)":"#1C2440",border:`0.5px solid ${corrigerFaibles?"#4D8BFF":"rgba(190,180,255,0.07)"}`,borderRadius:10,cursor:"pointer",marginTop:10,transition:"all.15s"}}>
          <div style={{width:20,height:20,borderRadius:5,background:corrigerFaibles?"#4D8BFF":"transparent",border:`1.5px solid ${corrigerFaibles?"#4D8BFF":"rgba(245,241,232,0.50)"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all.15s"}}>
          ...
          <div>
            ...
            <div style={{fontSize:10,color:"rgba(245,241,232,0.50)",marginTop:1}}>L'IA priorisera les groupes musculaires en retard détectés sur les photos</div>
          </div>
        </div>
