  ...
  return(
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'32px 20px 0'}}>
        ...
        {onSkip?(
          <button onClick={onSkip} className="tap" style={{padding:'8px 12px',borderRadius:999,background:C.s1,border:`1px solid ${C.bd}`,color:C.mid,fontSize:10,fontWeight:700,letterSpacing:0.4,fontFamily:DISPLAY,cursor:'pointer'}}>PASSER</button>
        ):<div style={{width:36}}/>}
      </div>
      <div style={{marginTop:16,padding:'0 20px'}}><Progress step={step} total={total}/></div>
      <div style={{padding:'24px 24px 0'}}>
        <div style={{fontSize:10,fontWeight:700,color:C.gold,letterSpacing:2,textTransform:'uppercase',fontFamily:DISPLAY}}>{eyebrow||`Étape ${step} sur ${total}`}</div>
        <div style={{fontFamily:SERIF,fontSize:34,fontWeight:400,letterSpacing:-1.2,color:C.text,lineHeight:1.05,marginTop:8}}>{title}</div>
...
const inputStyle={width:'100%',padding:'16px 16px',background:C.s2,border:`1px solid ${C.bdHi}`,borderRadius:16,fontSize:14,color:C.text,fontFamily:"'Inter',sans-serif",boxSizing:'border-box',boxShadow:'inset 0 1px 2px rgba(0,0,0,0.04)'};
  ...
  return(
    <div style={{background:`linear-gradient(160deg, ${accent}18 0%, ${accent}04 100%)`,border:`1px solid ${accent}30`,borderRadius:20,padding:20,position:'relative',overflow:'hidden',boxShadow:`0 2px 8px ${accent}20`}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:20,position:'relative'}}>
        <div style={{width:34,height:34,borderRadius:12,background:`linear-gradient(145deg, ${accent}, ${accent}99)`,color:'#0B1220',display:'grid',placeItems:'center',boxShadow:`0 4px 10px ${accent}40, inset 0 1px 0 rgba(0,0,0,0.2)`}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',position:'relative'}}>
        <button className="tap" onClick={()=>setValue(String(Math.max(min,v-1)))} style={{width:42,height:42,borderRadius:12,background:C.s2,border:`1px solid ${C.bdHi}`,color:C.text,display:'grid',placeItems:'center',padding:0,cursor:'pointer'}}>
        ...
        <button className="tap" onClick={()=>setValue(String(Math.min(max,v+1)))} style={{width:42,height:42,borderRadius:12,background:C.s2,border:`1px solid ${C.bdHi}`,color:C.text,display:'grid',placeItems:'center',padding:0,cursor:'pointer'}}>
            ...
            {oStep===3&&(
              <div style={{display:'flex',flexDirection:'column',gap:12}}>
                {[
                  ...
                  return(
                    <div key={a.id} onClick={()=>setOData({...oData,activite:a.id})} className="tap" style={{...selectCard(on,C.gold),padding:'16px 16px',borderRadius:16,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      <div>
