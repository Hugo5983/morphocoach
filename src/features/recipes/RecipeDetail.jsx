...
export default function RecipeDetail({ recipe, onBack, liked, onLike, push, repas, setRepas }) {
  ...
  return (
    <div className="anim" style={{ background:C.bg, minHeight:"100vh", paddingBottom:90, ...swipeStyle }}
        ...
        {showMealPicker && (
          <div onClick={() => setShowMealPicker(false)}
            ...
            <div onClick={e => e.stopPropagation()} style={{
              width:"100%", maxWidth:380, background:C.bg, borderRadius:22,
              padding:"22px 20px 18px",
              boxShadow:"0 24px 60px rgba(15,25,35,0.32)" }}>
              <div style={{ fontFamily:SERIF, fontSize:22, fontWeight:400,
                color:C.text, marginBottom:4, letterSpacing:-0.5 }}>
                Ajouter à quel repas ?
              </div>
              <div style={{ fontSize:12.5, color:C.dim, marginBottom:16, fontFamily:FONT }}>
                {r.nom} · {target} kcal
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {[
                  { id:"matin", l:"Petit-déjeuner", e:"☕", c:"#F59E0B" },
                  { id:"midi",  l:"Déjeuner",      e:"🍽️", c:"#3B82F6" },
                  ...
                  <button key={m.id} onClick={() => {
                    ...
                    <div style={{ width:38, height:38, borderRadius:11,
                      ...
                      display:"grid", placeItems:"center", fontSize:18 }}>{m.e}</div>
                    <div style={{ flex:1 }}>
