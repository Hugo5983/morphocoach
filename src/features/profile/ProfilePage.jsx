  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: "0 6px 18px -6px rgba(59,130,246,0.95)",
...
function WeightChart({ log }) {
    ...
    return (
      <Glass pad={0} style={{ animation: "rise .4s .05s both" }}>
        <div style={{ padding: "26px 20px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13.5, fontWeight: 700, color: C.text, fontFamily: FONT, marginBottom: 5 }}>
            Suivi du poids
          </div>
          <div style={{ fontSize: 12, color: "#6B7280", lineHeight: 1.5, fontFamily: FONT, maxWidth: 250, margin: "0 auto" }}>
  ...
  const deltaColor = delta < 0 ? "#34D399" : delta > 0 ? "#fb923c" : "#6B7280";
            ...
            <g key={i}>
              {(i === 0 || i === pts.length - 1) && (
                <circle cx={p.x} cy={p.y} r={i === pts.length - 1 ? 5 : 4}
                  fill={i === pts.length - 1 ? "#3B82F6" : "#fff"} stroke="#3B82F6" strokeWidth="2"/>
              )}
            </g>
          ))}
    ...
    <div className="anim" style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {(premium || premiumNutrition) && (
              <span style={{
                ...
                background: "linear-gradient(145deg, #3B82F6, #2563EB)",
                border: "1px solid rgba(37,99,235,0.6)",
                boxShadow: "0 4px 12px -3px rgba(59,130,246,0.9)",
                fontFamily: FONT,
              }}>
                PRO
              </span>
        ...
        <div key="compo">
          ...
          <Glass pad={0} style={{ animation: "rise .4s .14s both" }}>
            <div style={{ padding: "16px 18px" }}>
              ...
              {imc && (
                <div style={{ height: 6, borderRadius: 99, background: "linear-gradient(90deg,#34d399,#fbbf24,#f87171)", position: "relative" }}>
                  <div style={{ position: "absolute", top: -3, left: `${imcPct}%`, width: 12, height: 12, borderRadius: "50%", background: "#fff", border: "2px solid #cbd5e1", boxShadow: "0 2px 6px rgba(0,0,0,0.25)", transform: "translateX(-50%)" }}/>
                </div>
              )}
              <div style={{ marginTop: 12, fontSize: 11, color: "#9CA3AF", lineHeight: 1.5, fontFamily: FONT }}>
                ℹ️ L'IMC ne distingue pas muscle et graisse : il surestime souvent le « surpoids » chez les sportifs. Fie-toi davantage à ta <b>masse grasse</b> ci-dessus.
              </div>
            </div>
          </Glass>
        </div>
      ...
      {activeTab === "Mesures" && (
        <div key="mesures">
          ...
          <Glass pad={0} style={{ animation: "rise .4s .14s both" }}>
            <AddRow icon={ic.ruler} color="#34D399" label="Bras gauche" value={profil.mLeftArm}  onChange={set("mLeftArm")} />
            <AddRow icon={ic.ruler} color="#34D399" label="Bras droit"  value={profil.mRightArm} onChange={set("mRightArm")} last/>
          </Glass>

          <SectionLabel>Jambes</SectionLabel>
          <Glass pad={0} style={{ animation: "rise .4s .2s both" }}>
            <AddRow icon={ic.ruler} color="#fb923c" label="Cuisse gauche"  value={profil.mLeftThigh}  onChange={set("mLeftThigh")} />
            <AddRow icon={ic.ruler} color="#fb923c" label="Cuisse droite"  value={profil.mRightThigh} onChange={set("mRightThigh")} />
            <AddRow icon={ic.ruler} color="#fb923c" label="Mollet gauche"  value={profil.mLeftCalf}   onChange={set("mLeftCalf")} />
            <AddRow icon={ic.ruler} color="#fb923c" label="Mollet droit"   value={profil.mRightCalf}  onChange={set("mRightCalf")} last/>
          </Glass>
        </div>
      )}
