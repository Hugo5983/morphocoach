...
function SectionLabel({ children, icon }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "24px 4px 12px" }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "2.5px", color: "${C.dim}", fontFamily: FONT, textTransform: "uppercase" }}>{children}</span>
    </div>
  );
}
      ...
      {editing ? (
        options
          ? <select autoFocus value={value || ""} onChange={e => { onChange(e.target.value); setEditing(false); }}
              ...
              style={{ background: C.s2, border: "1px solid rgba(59,130,246,0.5)", borderRadius: 8, color: C.text, fontSize: 13, padding: "8px 12px", outline: "none" }}>
          ...
          : <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input autoFocus type={type} value={value || ""}
                ...
                style={{ background: C.s2, border: "1px solid rgba(59,130,246,0.5)", borderRadius: 8, color: C.text, fontSize: 13, padding: "8px 12px", outline: "none", maxWidth: 90, textAlign: "right" }}
              />
      ...
      {editing
        ? <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <input autoFocus type="number" value={value || ""}
              ...
              onBlur={() => setEditing(false)}
              onKeyDown={e => e.key === "Enter" && setEditing(false)}
              style={{ background: C.s2, border: "1px solid rgba(59,130,246,0.5)", borderRadius: 8, color: C.text, fontSize: 13, padding: "8px 12px", outline: "none", width: 70, textAlign: "right" }}
            />
...
function CompoBar({ icon, color, label, value, unit, onChange, pct, last }) {
  ...
  const pctVal = pct || 0;
  return (
    <div style={{ padding: "16px 20px", borderBottom: last ? "none" : "1px solid rgba(0,0,0,0.04)" }}>
        ...
        {editing
          ? <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <input autoFocus type="number"
                ...
                onBlur={() => setEditing(false)}
                onKeyDown={e => e.key === "Enter" && setEditing(false)}
                style={{ background: C.s2, border: "1px solid rgba(59,130,246,0.5)", borderRadius: 8, color: C.text, fontSize: 13, padding: "4px 8px", outline: "none", width: 70, textAlign: "right" }}
              />
          ...
          : <span onClick={() => setEditing(true)}
              ...
              {value ? `${value}${unit}` : <span style={{ fontSize: 13, color: "#60A5FA" }}>Ajouter</span>}
            </span>
        }
      </div>
...
function WeightChart({ log }) {
  ...
  const deltaColor = delta < 0 ? "#34D399" : delta > 0 ? "#FB923C" : C.dim;
    ...
    <Glass pad={0} style={{ animation: "rise .4s .05s both" }}>
      <div style={{ padding: "16px 16px 12px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "1.5px", color: C.dim, fontFamily: FONT, marginBottom: 4 }}>SUIVI DU POIDS</div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 10, color: "#9CA3AF", fontFamily: FONT }}>Depuis le {fmtShort(first.date)}</div>
        ...
        <div style={{ marginTop: 8, fontSize: 11, color: "#9CA3AF", fontFamily: FONT, textAlign: "center" }}>
