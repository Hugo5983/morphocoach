...
export function Spinner({ size = 40, color: c = color.primary, thickness = 3 }) {
  return (
    <div
      style={{
        width: size,
        height: size,
        border: `${thickness}px solid ${color.primarySoft}`,
...
export function LoadingBlock({ title, subtitle, spinnerSize = 56 }) {
  return (
    ...
    >
      ...
      {subtitle && (
        <div style={{ fontSize: 11, color: color.textMuted, lineHeight: 1.8 }}>
          {subtitle}
        </div>
      )}
    </div>
