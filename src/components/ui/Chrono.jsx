...
export function Chrono({onClose,initSec=90}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#0B1220",zIndex:400,display:"flex",flexDirection:"column",alignItems:"center",overflowY:"auto"}}>
      {/* Header */}
      <div style={{width:"100%",maxWidth:500,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px 0"}}>
        ...
        <div style={{display:"flex",background:"rgba(59,130,246,0.08)",borderRadius:10,padding:3,gap:3}}>
          {["countdown","stopwatch"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setLeft(total);setElapsed(0);setRun(false);setVibrated(false);}} style={{padding:"5px 8px",borderRadius:7,border:"none",background:mode===m?"#111827":"transparent",color:mode===m?"#4D8BFF":"rgba(242,244,247,0.50)",cursor:"pointer",fontSize:10,fontWeight:600,transition:"all .15s"}}>
              {m==="countdown"?"⏱":"⏲"}
            </button>
          ))}
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 20px 0",width:"100%",maxWidth:500}}>
          ...
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2}}>
            {done?(
              <>
                ...
                <div style={{fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",fontSize:16,fontWeight:500,color:"#5FE0A5",letterSpacing:1}}>C'EST PARTI !</div>
              </>
            ):(
              <>
                <div style={{fontFamily:"'Outfit','DM Sans',system-ui,sans-serif",fontSize:46,fontWeight:200,color:timeColor,letterSpacing:-2,lineHeight:1,transition:"color .3s"}}>
                  {mode==="countdown"?fmt(left):fmt(elapsed)}
                </div>
                <div style={{fontSize:10,color:"rgba(242,244,247,0.50)",fontWeight:500,letterSpacing:"1px",marginTop:2}}>
                  {mode==="countdown"
        ...
        <div style={{display:"flex",gap:16,alignItems:"center",marginBottom:24}}>
          {/* Reset */}
          <button onClick={()=>{setLeft(total);setElapsed(0);setRun(false);setVibrated(false);}} style={{width:52,height:52,borderRadius:"50%",background:"#111827",border:"0.5px solid rgba(255,255,255,0.07)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
          ...
          {/* +30s */}
          <button onClick={()=>{if(mode==="countdown"){setLeft(l=>l+30);setTotal(t=>t+30);}}} style={{width:52,height:52,borderRadius:"50%",background:"#111827",border:"0.5px solid rgba(255,255,255,0.07)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",flexDirection:"column",gap:1}}>
        ...
        {mode==="countdown"&&total>0&&(
          <div style={{width:"100%",maxWidth:280,marginBottom:24}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
              <span style={{fontSize:9,color:"rgba(242,244,247,0.50)"}}>0s</span>
              <span style={{fontSize:9,color:"rgba(242,244,247,0.50)"}}>{fmt(total)}</span>
            </div>
          </div>
        )}
      </div>
