...
import { C, INT, SESS_COLORS, SERIF } from "../../data/constants.js";
...
const ey      = { fontSize:10, fontWeight:700, letterSpacing:'1.3px', textTransform:'uppercase', color:C.accent, fontFamily:DISPLAY };
    ...
    <div>
      ...
      <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:12}}>
        {Object.entries(INT).map(([k,v])=>(
          <button key={k} onClick={()=>setInt(k)} style={{padding:'4px 12px',background:intensite===k?`${v.c}20`:C.s2,border:`1px solid ${intensite===k?v.c:'rgba(0,0,0,0.06)'}`,borderRadius:8,cursor:'pointer',fontSize:11,color:intensite===k?v.c:C.mid,fontWeight:intensite===k?700:400,fontFamily:DISPLAY}}>{v.l}</button>
        ))}
      </div>
      {exos.length>0&&(
      ...
      <div style={{border:'1px solid rgba(0,0,0,0.06)',borderRadius:12,overflow:'hidden',marginBottom:12}}>
        <div style={{padding:'8px',background:C.s2}}>
          <input value={search} onChange={e=>{setSearch(e.target.value);setGroupe(null);}} placeholder="Rechercher un exercice…"
            style={{width:'100%',padding:'8px 12px',background:'rgba(0,0,0,0.03)',border:'1px solid rgba(0,0,0,0.06)',borderRadius:8,fontSize:13,color:C.text,fontFamily:DISPLAY,outline:'none',boxSizing:'border-box'}}/>
        </div>
        {!search&&(
          <div style={{padding:'8px',display:'flex',flexWrap:'wrap',gap:4,maxHeight:110,overflowY:'auto'}}>
            {Object.keys(EX).map(g=>(
              <button key={g} onClick={()=>setGroupe(g===groupe?null:g)} style={{padding:'4px 8px',background:groupe===g?'rgba(59,130,246,0.10)':C.s2,border:`1px solid ${groupe===g?C.accent:'rgba(0,0,0,0.06)'}`,borderRadius:12,color:groupe===g?C.accent:C.mid,cursor:'pointer',fontSize:10,fontFamily:DISPLAY}}>
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
        <div style={{width:42,height:42,borderRadius:12,background:`${type.color}15`,border:`1px solid ${type.color}35`,display:'grid',placeItems:'center',flexShrink:0}}>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:12}}>
        {type.suggestions.map(s=>(
          <button key={s} onClick={()=>setNom(s)} style={{padding:'4px 12px',background:nom===s?`${type.color}18`:C.s2,border:`1px solid ${nom===s?type.color:'rgba(0,0,0,0.06)'}`,borderRadius:999,cursor:'pointer',fontSize:11,color:nom===s?type.color:C.mid,fontFamily:DISPLAY,fontWeight:nom===s?700:400}}>{s}</button>
        ))}
      </div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:12}}>
        <button onClick={()=>setDuree(d=>Math.max(10,d-5))} style={{width:34,height:34,borderRadius:8,background:C.s2,border:`1px solid rgba(0,0,0,0.06)`,cursor:'pointer',fontSize:20,color:C.mid,fontFamily:DISPLAY}}>−</button>
      ...
      <div style={{display:'flex',gap:4,marginBottom:16}}>
        {[15,20,30,45,60,90].map(d=>(
          <button key={d} onClick={()=>setDuree(d)} style={{flex:1,padding:'4px 2px',background:duree===d?`${type.color}14`:'transparent',border:`1px solid ${duree===d?type.color:'rgba(0,0,0,0.06)'}`,borderRadius:8,color:duree===d?type.color:C.mid,cursor:'pointer',fontSize:10,fontFamily:DISPLAY,fontWeight:duree===d?700:400}}>{d}'</button>
        ))}
      </div>
    ...
    <div style={overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={sheet}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
          ...
          <button onClick={onClose} style={{width:32,height:32,borderRadius:8,background:'rgba(255,255,255,0.07)',border:'1px solid rgba(170,180,255,0.22)',color:C.mid,cursor:'pointer',display:'grid',placeItems:'center'}}>
        ...
        {/* Musculation CTA */}
        <button onClick={()=>setStep('musculation')} style={{width:'100%',display:'flex',alignItems:'center',gap:12,padding:'12px 16px',background:C.s2,border:`1px solid rgba(59,130,246,0.22)`,borderRadius:16,cursor:'pointer',marginBottom:12,textAlign:'left'}}>
          <div style={{width:42,height:42,borderRadius:12,background:'rgba(59,130,246,0.12)',display:'grid',placeItems:'center',flexShrink:0}}>
    ...
    <div style={{marginTop:24}}>
      ...
      <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:16}}>
        ...
        {currentWeek !== undefined && (
          <div style={{fontSize:10,fontWeight:700,color:'#60A5FA',fontFamily:DISPLAY,letterSpacing:'1px',textTransform:'uppercase',paddingBottom:2}}>
      ...
      <div style={{display:'flex',gap:8}}>
        ...
        {/* Séances */}
        <div style={{flex:1,background:C.s1,border:'1px solid rgba(0,0,0,0.06)',borderRadius:16,padding:'16px 12px',minWidth:0}}>
          <div style={{...ey,marginBottom:12}}>Séances</div>
          <div style={{fontFamily:DISPLAY,fontSize:26,fontWeight:700,color:C.text,letterSpacing:-1,lineHeight:1,...NUM}}>
            {stats.validated}
        ...
        {/* Assiduité */}
        <div style={{flex:1,background:C.s1,border:'1px solid rgba(0,0,0,0.06)',borderRadius:16,padding:'16px 12px',minWidth:0}}>
          <div style={{...ey,marginBottom:12}}>Assiduité</div>
          <div style={{fontFamily:DISPLAY,fontSize:26,fontWeight:700,color:C.text,letterSpacing:-1,lineHeight:1,...NUM}}>
            {stats.assiduite}
            <span style={{fontSize:13,fontWeight:400,color:'#9CA3AF',marginLeft:1}}>%</span>
          </div>
          <div style={{height:7,background:'rgba(0,0,0,0.05)',borderRadius:5,overflow:'hidden',marginTop:8}}>
...
export const MonthCal = memo(function MonthCal({ sessions, onUpdate, semC, currentWeek }) {
  ...
  return (
    <div style={{paddingBottom:4}}>
      ...
      {/* ── Calendrier card ── */}
      <div style={{background:`radial-gradient(ellipse 90% 60% at 85% 0%, rgba(99,72,235,0.35), transparent 55%), radial-gradient(ellipse 70% 50% at 10% 100%, rgba(47,107,255,0.22), transparent 60%), linear-gradient(160deg, #0E0C28 0%, #12102F 55%, #191243 100%)`,border:'1px solid rgba(170,180,255,0.16)',boxShadow:'0 12px 32px rgba(12,14,34,0.45)',borderRadius:20,padding:'16px 16px 16px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
          <button onClick={()=>canPrev&&setDate(new Date(y,m-1,1))} disabled={!canPrev} className="tap"
            style={{width:30,height:30,borderRadius:8,background:'rgba(255,255,255,0.07)',border:'1px solid rgba(170,180,255,0.22)',color:canPrev?C.mid:C.dim,cursor:canPrev?'pointer':'not-allowed',display:'grid',placeItems:'center',padding:0,flexShrink:0}}>
            <I name="chevL" size={13} stroke={2} color={canPrev?'#C3C8E6':'#565D85'}/>
          </button>
          <div style={{textAlign:'center'}}>
            <div style={{...ey,color:'#9AA6FF',marginBottom:4}}>Calendrier</div>
          ...
          <button onClick={()=>canNext&&setDate(new Date(y,m+1,1))} disabled={!canNext} className="tap"
            style={{width:30,height:30,borderRadius:8,background:'rgba(255,255,255,0.07)',border:'1px solid rgba(170,180,255,0.22)',color:canNext?C.mid:C.dim,cursor:canNext?'pointer':'not-allowed',display:'grid',placeItems:'center',padding:0,flexShrink:0}}>
            <I name="chevR" size={13} stroke={2} color={canNext?'#C3C8E6':'#565D85'}/>
          </button>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:4}}>
          {DAYS.map((d,i)=>(
            <div key={i} style={{textAlign:'center',fontSize:10,color:'#8F95B8',fontWeight:700,letterSpacing:0.8,fontFamily:DISPLAY}}>{d}</div>
          ))}
        </div>
        ...
        <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid rgba(170,180,255,0.14)',display:'flex',flexWrap:'wrap',gap:8,alignItems:'center'}}>
          ...
          {Object.entries(INT).map(([k,v])=>(
            <div key={k} style={{display:'flex',alignItems:'center',gap:4}}>
              ...
              <span style={{fontSize:10,color:'#C3C8E6',fontFamily:DISPLAY}}>{v.l}</span>
            </div>
          ))}
        </div>
        <div style={{marginTop:8,display:'flex',flexWrap:'wrap',gap:12,alignItems:'center'}}>
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            ...
            <span style={{fontSize:10,color:'#C3C8E6',fontFamily:DISPLAY}}>Planifiée</span>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:4}}>
            ...
            <span style={{fontSize:10,color:'#C3C8E6',fontFamily:DISPLAY}}>Validée (faite)</span>
          </div>
        </div>
      </div>
