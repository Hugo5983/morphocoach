      ...
      <div style={{width:"100%",maxWidth:500,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px 0"}}>
        ...
        <div style={{display:"flex",background:"rgba(59,130,246,0.08)",borderRadius:10,padding:3,gap:3}}>
          {["countdown","stopwatch"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setLeft(total);setElapsed(0);setRun(false);setVibrated(false);}} style={{padding:"5px 8px",borderRadius:7,border:"none",background:mode===m?"#FFFFFF":"transparent",color:mode===m?"#4D8BFF":"rgba(15,25,35,0.50)",cursor:"pointer",fontSize:10,fontWeight:600,transition:"all .15s"}}>
              {m==="countdown"?"⏱":"⏲"}
            </button>
          ))}
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 20px 0",width:"100%",maxWidth:500}}>
          ...
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2}}>
            {done?(
              ...
              <>
                ...
                <div style={{fontSize:10,color:"rgba(15,25,35,0.50)",fontWeight:500,letterSpacing:"1px",marginTop:2}}>
                  {mode==="countdown"
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",marginBottom:24}}>
            {PRESETS.map(p=>(
              <button key={p.s} onClick={()=>handlePreset(p.s)} style={{
                padding:"8px 14px",
                background:total===p.s&&!done?"#4D8BFF":"#FFFFFF",
                border:`1px solid ${total===p.s&&!done?"#4D8BFF":"rgba(0,0,0,0.06)"}`,
                borderRadius:16,
                color:total===p.s&&!done?"#FFFFFF":"rgba(15,25,35,0.50)",
        ...
        <div style={{display:"flex",gap:16,alignItems:"center",marginBottom:24}}>
          ...
          <button onClick={()=>{setLeft(total);setElapsed(0);setRun(false);setVibrated(false);}} style={{width:52,height:52,borderRadius:"50%",background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(15,25,35,0.50)" strokeWidth="2" strokeLinecap="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.8"/></svg>
          </button>
          ...
          <button onClick={()=>{if(mode==="countdown"){setLeft(l=>l+30);setTotal(t=>t+30);}}} style={{width:52,height:52,borderRadius:"50%",background:C.s1,border:"0.5px solid rgba(0,0,0,0.06)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",flexDirection:"column",gap:1}}>
            ...
            <span style={{fontSize:8,color:"rgba(15,25,35,0.50)"}}>sec</span>
          </button>
        </div>
        ...
        {mode==="countdown"&&total>0&&(
          <div style={{width:"100%",maxWidth:280,marginBottom:24}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
              <span style={{fontSize:9,color:"rgba(15,25,35,0.50)"}}>0s</span>
              <span style={{fontSize:9,color:"rgba(15,25,35,0.50)"}}>{fmt(total)}</span>
            </div>
          </div>
        )}
      </div>
