    ...
    <div>
      ...
      <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:12}}>
        {Object.entries(INT).map(([k,v])=>(
          <button key={k} onClick={()=>setInt(k)} style={{padding:'5px 10px',background:intensite===k?`${v.c}20`:C.s2,border:`1px solid ${intensite===k?v.c:'rgba(0,0,0,0.06)'}`,borderRadius:7,cursor:'pointer',fontSize:11,color:intensite===k?v.c:'rgba(15,25,35,0.50)',fontWeight:intensite===k?700:400,fontFamily:DISPLAY}}>{v.l}</button>
        ))}
      </div>
      {exos.length>0&&(
      ...
      <div style={{border:'1px solid rgba(0,0,0,0.06)',borderRadius:10,overflow:'hidden',marginBottom:12}}>
        ...
        {!search&&(
          <div style={{padding:'8px',display:'flex',flexWrap:'wrap',gap:4,maxHeight:110,overflowY:'auto'}}>
            {Object.keys(EX).map(g=>(
              <button key={g} onClick={()=>setGroupe(g===groupe?null:g)} style={{padding:'4px 9px',background:groupe===g?'rgba(59,130,246,0.10)':C.s2,border:`1px solid ${groupe===g?'#3B82F6':'rgba(0,0,0,0.06)'}`,borderRadius:12,color:groupe===g?'#3B82F6':'rgba(15,25,35,0.45)',cursor:'pointer',fontSize:10,fontFamily:DISPLAY}}>
        ...
        {!search&&!groupe&&<div style={{padding:'12px',textAlign:'center',fontSize:11,color:'rgba(15,25,35,0.40)'}}>Sélectionne un groupe musculaire</div>}
      </div>
    ...
    <div>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        ...
        <div>
          ...
          <div style={{fontSize:10.5,color:'rgba(15,25,35,0.45)',marginTop:1}}>{type.desc}</div>
        </div>
      </div>
      <div style={{display:'flex',flexWrap:'wrap',gap:5,marginBottom:12}}>
        {type.suggestions.map(s=>(
          <button key={s} onClick={()=>setNom(s)} style={{padding:'5px 10px',background:nom===s?`${type.color}18`:C.s2,border:`1px solid ${nom===s?type.color:'rgba(0,0,0,0.06)'}`,borderRadius:999,cursor:'pointer',fontSize:10.5,color:nom===s?type.color:'rgba(15,25,35,0.50)',fontFamily:DISPLAY,fontWeight:nom===s?700:400}}>{s}</button>
        ))}
      </div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
        <button onClick={()=>setDuree(d=>Math.max(10,d-5))} style={{width:34,height:34,borderRadius:9,background:C.s2,border:`1px solid rgba(0,0,0,0.06)`,cursor:'pointer',fontSize:18,color:'rgba(15,25,35,0.50)',fontFamily:DISPLAY}}>−</button>
        <div style={{flex:1,textAlign:'center',fontFamily:DISPLAY,fontSize:24,fontWeight:700,color:C.text,...NUM}}>{duree}<span style={{fontSize:12,fontWeight:400,color:'rgba(15,25,35,0.40)',marginLeft:4}}>min</span></div>
      ...
      <div style={{display:'flex',gap:5,marginBottom:16}}>
        {[15,20,30,45,60,90].map(d=>(
          <button key={d} onClick={()=>setDuree(d)} style={{flex:1,padding:'5px 2px',background:duree===d?`${type.color}14`:'transparent',border:`1px solid ${duree===d?type.color:'rgba(0,0,0,0.06)'}`,borderRadius:7,color:duree===d?type.color:'rgba(15,25,35,0.40)',cursor:'pointer',fontSize:10,fontFamily:DISPLAY,fontWeight:duree===d?700:400}}>{d}'</button>
        ))}
      </div>
      ...
      <button onClick={onBack} style={{width:'100%',marginTop:8,padding:'8px',background:'transparent',border:'none',color:'rgba(15,25,35,0.45)',cursor:'pointer',fontSize:12,fontFamily:DISPLAY}}>← Retour</button>
    </div>
  );
}
    ...
    <div style={overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={sheet}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
          ...
          <button onClick={onClose} style={{width:32,height:32,borderRadius:9,background:C.s2,border:`1px solid ${C.bd}`,color:'rgba(15,25,35,0.50)',cursor:'pointer',display:'grid',placeItems:'center'}}>
              ...
              <div key={i} style={{padding:'10px 12px',background:sess.done?`${col}14`:C.s2,borderRadius:12,marginBottom:6,borderLeft:`3px solid ${col}`,border:sess.done?`1px solid ${col}40`:'1px solid transparent'}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:10.5,color:'rgba(15,25,35,0.45)',marginTop:2}}>{INT[sess.intensite||'modere']?.l}{sess.bonus?` · ${sess.bonus.duree}min`:''}{sess.done?' · Validée ✓':''}</div>
                  </div>
                ...
                {onToggleDone&&(
                  <button onClick={()=>onToggleDone(i)} style={{
                    ...
                    color:sess.done?'rgba(15,25,35,0.55)':'#fff',
                  }}>
        ...
        <button onClick={()=>setStep('musculation')} style={{width:'100%',display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:C.s2,border:`1px solid rgba(59,130,246,0.22)`,borderRadius:14,cursor:'pointer',marginBottom:10,textAlign:'left'}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:10.5,color:'rgba(15,25,35,0.45)',marginTop:1}}>Exercices, séries, charges</div>
          </div>
          <I name="chevR" size={14} color='rgba(15,25,35,0.25)' stroke={2}/>
        </button>
...
function BilanMois({ sessions, year, month, cycleStart, currentWeek }) {
  ...
  const chargeBarre = topIntData ? (
    <div>
      ...
      <div style={{fontSize:11,color:'rgba(15,25,35,0.40)',fontFamily:DISPLAY,marginTop:6}}>{CHARGE_SUB[topIntKey]||''}</div>
    </div>
  ) : (
    ...
    <div style={{paddingBottom:4}}>
      ...
      <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:18,padding:'16px 14px 14px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:3,marginBottom:5}}>
          {DAYS.map((d,i)=>(
            <div key={i} style={{textAlign:'center',fontSize:9,color:'rgba(15,25,35,0.28)',fontWeight:700,letterSpacing:0.8,fontFamily:DISPLAY}}>{d}</div>
          ))}
        </div>
        ...
        <div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${C.bd}`,display:'flex',flexWrap:'wrap',gap:8,alignItems:'center'}}>
          ...
          {Object.entries(INT).map(([k,v])=>(
            <div key={k} style={{display:'flex',alignItems:'center',gap:4}}>
              ...
              <span style={{fontSize:9.5,color:'rgba(15,25,35,0.40)',fontFamily:DISPLAY}}>{v.l}</span>
            </div>
          ))}
        </div>
        <div style={{marginTop:9,display:'flex',flexWrap:'wrap',gap:12,alignItems:'center'}}>
          <div style={{display:'flex',alignItems:'center',gap:5}}>
            ...
            <span style={{fontSize:9.5,color:'rgba(15,25,35,0.40)',fontFamily:DISPLAY}}>Planifiée</span>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:5}}>
            ...
            <span style={{fontSize:9.5,color:'rgba(15,25,35,0.40)',fontFamily:DISPLAY}}>Validée (faite)</span>
          </div>
        </div>
      </div>
