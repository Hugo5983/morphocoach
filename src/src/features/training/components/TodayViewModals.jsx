    ...
    <div style={overlayStyle} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={sheetStyle}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 20px 16px"}}>
          <div>
            <div style={{fontSize:9,color:"rgba(15,25,35,0.40)",fontWeight:700,letterSpacing:"1.4px",textTransform:"uppercase",marginBottom:5,fontFamily:DISP_F}}>Nouveau record</div>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
                    {groupes.map(g => (
                      <button key={g} onClick={() => setGroupe(g===groupe?null:g)}
                        style={{padding:"9px 16px",background:groupe===g?"rgba(59,130,246,0.12)":C.s1,border:`1.5px solid ${groupe===g?"#3B82F6":"rgba(0,0,0,0.06)"}`,borderRadius:12,color:groupe===g?"#60A5FA":"rgba(15,25,35,0.55)",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:DISP_F,whiteSpace:"nowrap",transition:"all .18s",flexShrink:0}}>
          ...
          {!newExForm && step===1 && (<>
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:20}}>
              {Object.entries(INT).map(([k,v])=>{const on=intensite===k;return(
                <button key={k} onClick={()=>setInt(k)} style={{padding:"9px 14px",borderRadius:12,border:`1px solid ${on?v.c:C.bd}`,background:on?`${v.c}18`:C.s1,color:on?v.c:"rgba(15,25,35,0.55)",fontFamily:DISP_F,fontSize:12,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:7}}>
            ...
            <div style={{display:"flex",gap:7,marginBottom:20}}>
              {DURS.map(d=>{const on=duree===d;return(
                <button key={d} onClick={()=>setDuree(d)} style={{flex:1,padding:"11px 0",borderRadius:12,border:`1px solid ${on?"#3B82F6":C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#60A5FA":"rgba(15,25,35,0.55)",fontFamily:DISP_F,fontSize:11,fontWeight:600,cursor:"pointer",textAlign:"center"}}>{d}</button>
              );})}
            </div>
          </>)}
          ...
          {newExForm && (<>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,padding:"13px 15px",background:`${cc(newExForm.cat)}0d`,border:`1px solid ${cc(newExForm.cat)}30`,borderRadius:14,marginBottom:16,marginTop:4}}>
              ...
              <div>
                ...
                <div style={{fontSize:10.5,color:"rgba(15,25,35,0.40)",marginTop:2,fontFamily:DISP_F}}>{newExForm.group}</div>
              </div>
            </div>
            ...
            <div style={{background:C.s1,border:`1px solid ${C.bd}`,borderRadius:16,padding:"14px",marginBottom:12}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                {METHODS.map(mm=>{const on=newExForm.methode===mm;return(
                  <button key={mm} onClick={()=>setNewExForm(f=>({...f,methode:mm}))} style={{padding:"5px 11px",borderRadius:99,border:`1px solid ${on?"#3B82F6":C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#60A5FA":"rgba(15,25,35,0.45)",cursor:"pointer",fontSize:10.5,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
                );})}
              </div>
            </div>
          </>)}
                    ...
                    <div style={{borderTop:`1px solid ${C.bd}`,padding:"12px 14px",background:"rgba(59,130,246,0.03)"}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                        {METHODS.map(mm=>{const on=ex.methode===mm;return(
                          <button key={mm} onClick={()=>updateField(i,"methode",mm)} style={{padding:"4px 10px",borderRadius:99,border:`1px solid ${on?"#3B82F6":C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#60A5FA":"rgba(15,25,35,0.45)",cursor:"pointer",fontSize:10,fontWeight:on?700:500,fontFamily:DISP_F}}>{mm}</button>
                        );})}
                      </div>
                    </div>
                  )}
            ...
            {!search && (
              <div style={{display:"flex",gap:8,overflowX:"auto",padding:"0 0 12px",flexShrink:0,scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
                {Object.keys(EX).map(g=>{const on=groupe===g;return(
                  <button key={g} onClick={()=>setGroupe(g===groupe?null:g)} style={{flexShrink:0,padding:"9px 16px",borderRadius:12,border:`1.5px solid ${on?"#3B82F6":C.bd}`,background:on?"rgba(59,130,246,0.12)":C.s1,color:on?"#3B82F6":"rgba(15,25,35,0.55)",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:DISP_F,whiteSpace:"nowrap"}}>{g}</button>
                );})}
              </div>
            )}
            ...
            {searchList.length>0 && searchList.map((ex,i)=>{
              ...
              return (
                <div key={i} style={{background:C.s1,border:`1px solid ${done?"rgba(52,211,153,0.25)":C.bd}`,borderRadius:16,padding:"16px",marginBottom:10,borderLeft:`3px solid ${col}`,boxShadow:`0 8px 24px -16px ${col}`}}>
                  ...
                  <div style={{fontSize:13,color:"rgba(15,25,35,0.55)",marginBottom:4,fontFamily:DISP_F}}>
          ...
          <div style={{display:"flex",gap:9}}>
            {(step===2||newExForm) && (
              <button onClick={newExForm?()=>setNewExForm(null):()=>setStep(1)}
                style={{flex:1,padding:"15px",borderRadius:14,border:`1px solid ${C.bd}`,background:"transparent",color:"rgba(15,25,35,0.45)",fontFamily:DISP_F,fontSize:14,fontWeight:600,cursor:"pointer"}}>
                ← Retour
              </button>
            )}
      ...
      <div style={{...SHEET_BASE, maxHeight:"88vh", overflowY:"auto", WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"6px 20px 14px"}}>
          <div>
            ...
            <div style={{fontSize:11,color:"rgba(15,25,35,0.45)",marginTop:4,fontFamily:DISP_F}}>1RM actuel : <span style={{color:"#60A5FA",fontWeight:700}}>{exData.rm1} kg</span></div>
          </div>
            ...
            <div>
              ...
              <div style={{borderRadius:16,padding:"14px 16px",marginBottom:14,background:"linear-gradient(135deg,rgba(59,130,246,0.14),rgba(37,99,235,0.05))",border:"1px solid rgba(59,130,246,0.22)",display:"flex",alignItems:"center",justifyContent:"space-between",opacity:rm1Edit>0?1:0.45,transition:"opacity .25s"}}>
                <div>
                  ...
                  <div style={{fontSize:11,color:"rgba(15,25,35,0.55)",fontFamily:DISP_F}}>{rm1Edit>0?(parseInt(editReps)===1?"Ta charge max sur 1 rep":"Estimation Epley"):"Saisis charge et reps"}</div>
                </div>
              ...
              <div style={{display:"flex",gap:9}}>
                <button onClick={cancelEdit} style={{flex:1,padding:"13px",borderRadius:14,border:`1px solid ${C.bd}`,background:"transparent",color:"rgba(15,25,35,0.45)",fontFamily:DISP_F,fontSize:13,fontWeight:600,cursor:"pointer"}}>Annuler</button>
                ...
                return (
                  <div key={i} style={{background:C.s1,border:`1px solid ${isRecord?"rgba(96,165,250,0.3)":C.bd}`,borderRadius:14,padding:"13px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
                    <div style={{flex:1,cursor:"pointer",minWidth:0}} onClick={()=>openEdit(i)}>
                      ...
                      <div style={{fontSize:10,color:"rgba(15,25,35,0.40)",fontFamily:DISP_F}}>{h.date} · 1RM : <span style={{color:"#60A5FA",fontWeight:600}}>{rm} kg</span> · <span style={{color:"#60A5FA"}}>Appuie pour modifier</span></div>
                    </div>
