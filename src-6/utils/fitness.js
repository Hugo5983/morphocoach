...
export function getBFCategory(bf, sexe) {
  if (sexe === "femme") {
    if (bf < 14) return { label: "Athlète ⚡", color: "#5FE0A5" };
    if (bf < 21) return { label: "Forme ✅",   color: "#5FE0A5" };
    if (bf < 25) return { label: "Acceptable", color: "#FFAB5D" };
    if (bf < 32) return { label: "À améliorer",color: "#FFAB5D" };
    return { label: "Obésité", color: "#FF7A6B" };
  }
  if (bf < 6)  return { label: "Athlète ⚡", color: "#5FE0A5" };
  if (bf < 14) return { label: "Forme ✅",   color: "#5FE0A5" };
  if (bf < 18) return { label: "Acceptable", color: "#FFAB5D" };
  if (bf < 25) return { label: "À améliorer",color: "#FFAB5D" };
  return { label: "Obésité", color: "#FF7A6B" };
}
