    ...
    return (
      <div style={{minHeight:"100vh",background:C.bg}}>
        <div style={{padding:"20px 16px 32px"}}>
          <button onClick={()=>setNewExForm(null)} style={{background:"transparent",border:"none",color:DARK.accent,cursor:"pointer",fontSize:14,fontWeight:600,fontFamily:DISP_F,display:"flex",alignItems:"center",gap:4,marginBottom:16}}>
  ...
  if (showBiblio) {
    return (
      <div style={{minHeight:"100vh",background:C.bg}}>
        <div style={{padding:"20px 16px 32px"}}>
          {/* Back */}
          <button onClick={()=>{setShowBiblio(false);setSearch("");setGroupe(null);}} style={{background:"transparent",border:"none",color:DARK.accent,cursor:"pointer",fontSize:14,fontWeight:600,fontFamily:DISP_F,display:"flex",alignItems:"center",gap:4,marginBottom:16}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
            Retour à la séance
          </button>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <div style={{paddingBottom:32}}>
        ...
        <div style={{padding:"16px 16px 0"}}>
          {/* Hero séance */}
          <div style={{position:"relative",borderRadius:28,overflow:"hidden",marginBottom:16,padding:"20px 20px 20px",background:`linear-gradient(155deg,${int.c} 0%,${int.c}cc 55%,${int.c}66 100%)`,border:"1px solid rgba(0,0,0,0.12)",boxShadow:`0 20px 40px ${int.c}40`}}>
            <div style={{position:"absolute",inset:0,pointerEvents:"none",background:"radial-gradient(160% 60% at 20% 10%,rgba(0,0,0,0.18),transparent 55%)"}}/>
            <div style={{position:"relative"}}>
              <div style={{display:"inline-flex",alignItems:"center",gap:4,padding:"4px 8px",borderRadius:999,background:"rgba(11,15,31,0.25)",color:DARK.bg,fontSize:10,fontWeight:700,fontFamily:DISP_F,letterSpacing:"0.1em",marginBottom:16}}>
                <span style={{width:6,height:6,borderRadius:"50%",background:DARK.bg}}/>
                {int.l.toUpperCase()}
              </div>
              <div style={{fontFamily:SERIF_F,fontSize:34,fontWeight:400,letterSpacing:-1,color:DARK.bg,lineHeight:1,marginBottom:8}}>{localName}</div>
              <div style={{fontSize:13,color:"rgba(11,15,31,0.85)",fontWeight:600,fontFamily:DISP_F,marginBottom:16}}>{jour.focus}</div>
              ...
              <div key={k} style={{background:C.s1,border:`1px solid ${isEditing?colour+"40":C.bd}`,borderRadius:16,marginBottom:8,overflow:"hidden"}}>
                <div style={{padding:"12px 16px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:40,height:40,borderRadius:12,flexShrink:0,background:`linear-gradient(145deg,${colour}30,${colour}08)`,border:`1px solid ${colour}40`,color:colour,display:"grid",placeItems:"center",fontFamily:DISP_F,fontSize:13,fontWeight:700}}>{k+1}</div>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                        {[{l:"Séries",k:"series"},{l:"Reps",k:"reps"},{l:"Repos",k:"repos"},{l:"Charge",k:"charge"}].map(pp=>(
                          <div key={pp.k} style={{minWidth:0}}>
                            <div style={{fontSize:8,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"${C.dim}",marginBottom:4,fontFamily:DISP_F}}>{pp.l}</div>
